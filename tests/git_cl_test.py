...
class ChangelistTest(unittest.TestCase):
  ...
  def testGetLocalDescription(self, _mock):
    git_cl.RunGit.side_effect = ['description', 'description no-cache']
    cl = git_cl.Changelist()
    self.assertEqual('description', cl.GetLocalDescription(['branch']))
    self.assertEqual('description', cl.GetLocalDescription(['branch']))
    self.assertEqual(
        'description no-cache',
        cl.GetLocalDescription(['branch'], use_cache=False))
    self.assertEqual(
        [
            mock.call(['log', '--pretty=format:%s%n%n%b', 'branch..']),
            mock.call(['log', '--pretty=format:%s%n%n%b', 'branch..']),
        ],
        git_cl.RunGit.mock_calls)
