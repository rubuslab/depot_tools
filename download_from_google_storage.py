            ...
            if full_path.endswith(".sha1"):
                if auto_platform:
                    ...
                    if not target_platform:
                        err = (
                            "--auto_platform passed in but no platform name found in "
...
def main(args):
    ...
    if options.no_auth:
        if (
            set(("http_proxy", "https_proxy")).intersection(
                env.lower() for env in os.environ
            )
            and "NO_AUTH_BOTO_CONFIG" not in os.environ
        ):
            print(
                "NOTICE: You have PROXY values set in your environment, but gsutil"
            ...
            print(
                "To use a proxy in this situation, please supply those settings"
