...
class GitApi(recipe_api.RecipeApi):
    ...
    def cat_file_at_commit(
        ...
        kwargs.setdefault(
            "name", "git cat-file %s:%s" % (commit_hash, file_path)
        )
        return self(
        ...
        with self.m.context(cwd=dir_path):
            try:
                ...
                self(
                    "rebase",
                    "--abort",
                    name="%s rebase abort" % name_prefix,
                    **kwargs
                )
                raise
    ...
    def new_branch(
        ...
        if upstream and upstream_current:
            raise ValueError(
                "Can not define both upstream and upstream_current"
            )
