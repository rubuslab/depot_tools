...
class _GitNumbererState(object):
  ...
  def load(cls, remote_url, remote_ref):
    ...
    if error:
      ...
      # with git-numberer.
      logging.debug('failed to fetch gnumbd and project config for %s: %s',
                    remote_url, error)
      return cls(cls._get_pending_prefix_fallback(), None)
  ...
  def _get_pending_prefix(cls, ref):
    ...
    try:
      ...
      if any(cls.match_refglob(ref, e) for e in config['enabled_refglobs']):
        return config['pending_ref_prefix']
      return None
    except KeyboardInterrupt:
      raise
    except Exception as e:
      logging.warn('failed to parse gnumbd config: %s', e)
      return cls._get_pending_prefix_fallback()
  ...
  def _is_validator_enabled(cls, ref):
    ...
    if any(cls.match_refglob(ref, d) for d in disabled):
      return False
    return any(cls.match_refglob(ref, e) for e in enabled)

  @staticmethod
  def match_refglob(ref, refglob):
    if ref == refglob:
      return True
    return fnmatch.fnmatch(ref, refglob)
  ...
  def CMDUploadChange(self, options, args, change):
    ...
    if remote_url:
      ...
      target_ref = GetTargetRef(remote, remote_branch, options.target_branch,
                                _GitNumbererState.load().pending_prefix)
      if target_ref:
