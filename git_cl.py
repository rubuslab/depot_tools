...
def UploadAll(options, args, orig_args):
  ...
  if upload_all:
    ...
    for cl in reversed(cls):
      ...
      # Get watchlist and presubmit ccs for all branches seperately
      files = cls[0].GetAffectedFiles(parent)
      extra_ccs = watchlist.GetWatchersForPaths(files)
      if not options.bypass_hooks:
        hook_results = self.RunHook(
            committing=False,
            may_prompt=not options.force,
            ...
            resultdb=options.resultdb,
            realm=options.realm)
        extra_ccs.extend(hook_results['more_cc'])
      ...
      parent = rev_to_commit = _GetSquashedCommit(cl, parent, change_desc)
...
def _GetSquashedCommit(cl, parent, change_desc):
  """."""
  ...
  if last_gerrit_commit and (RunGitSilent(
    ...
    # Nothing to commit since last time # TODO: this should not happen
    # TODO check current branch actually has stuff to commit.
    return None
