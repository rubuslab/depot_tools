...
class VersionField(_FreeformTextMetadataField):
  ...
  def is_unknown(value: str) -> bool:
    """Checks if the value actually denotes the version being unknown."""
...
class _YesNoMetadataField(_MetadataField):
  ...
  @classmethod
  def validate(cls, value: str) -> Union[ValidationResult, None]:
    """Checks the given value is acceptable - should be either "yes" or "no".
    """
...
class URLField(_MetadataField):
  ...
  @classmethod
  def validate(cls, value: str) -> Union[ValidationResult, None]:
    """Checks the given value is acceptable. Note: this field supports multiple
...
class DateField(_MetadataField):
  ...
  @classmethod
  def validate(cls, value: str) -> Union[ValidationResult, None]:
    """Checks the given value is acceptable - must use format YYYY-MM-DD.
    """
...
class LicenseField(_MetadataField):
  ...
  def validate(cls, value: str) -> Union[ValidationResult, None]:
    """Checks the given value is acceptable. Note: this field supports multiple
    values with comma separation. Each license must be in the allowlist of
    licenses.
    """
    invalid_values = []
...
class LicenseFileField(_MetadataField):
  ...
  def validate(cls, value: str) -> Union[ValidationResult, None]:
    """Checks the given value is acceptable. Note: this field supports multiple
    values with comma separation. This validation is rudimentary, and simply
    checks that each license file has at least one non-whitespace character.

    To check if the license file(s) exist on disk, see the
    `LicenseFileField.validate_on_disk` method.
    """
  ...
  def validate_on_disk(cls, value: str, readme_dir: str,
    ...
    """Checks the given value is acceptable, and that each license file exists
    on disk. Note: this field supports multiple values with comma separation.
    """
    if value == "NOT_SHIPPED":
      return ValidationWarning(
          f"{cls.NAME} has deprecated value NOT_SHIPPED "
...
class CPEPrefixField(_MetadataField):
  ...
  @classmethod
  def validate(cls, value: str) -> Union[ValidationResult, None]:
    """Checks the given value is acceptable - must be 'unknown' or a valid
