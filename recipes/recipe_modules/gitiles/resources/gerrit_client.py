...
def main(arguments):
    ...
    if not parsed_url.scheme.startswith("http"):
        parser.error(
            "Invalid URI scheme (expected http or https): %s" % args.url
        )
    ...
    if query_params.get("format"):
        parser.error(
            "URL must not contain format; use --format command line flag "
            "instead."
        )
    ...
    accept_statuses = frozenset(
        [int(s) for s in args.accept_statuses.split(",")]
    )
    if accept_statuses:
        ...
        def handler(conn):
            ...
            with tarfile.open(mode="r|*", fileobj=fileobj) as tf:
                ...
                def _extract_member(tarinfo, targetpath):
                    if not os.path.abspath(targetpath).startswith(
                        args.extract_to
                    ):
    ...
    def fetch(query_params):
        ...
        if not args.quiet:
            logging.info(
                "Read from %s: %s", parsed_url_with_query.geturl(), result
            )
        return result
...
def create_argparser():
    ...
    parser.add_argument(
        "-j", "--json-file", help="Path to json file for output."
    )
    parser.add_argument(
        "--extract-to",
