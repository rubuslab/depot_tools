...
class GitWrapper(SCMWrapper):
  ...
  def apply_patch_ref(self, patch_repo, patch_rev, target_rev, options,
    ...
    # Other option is parse the patch_ref to get the cl and patchset out
    # and then use gerrit_util to do it.
    if options.download_topics:
      ...
      patchset = patch_rev_tokens[-1]
      print('I GOT THIS!')
      print(change)
      print(patchset)
      print(patch_repo)
      print(self.url)
      # Get the topic of the provided change first.
      change_object = gerrit_util.GetChange('skia-review.googlesource.com', change)
      topic = change_object.get('topic')
      if topic:
        print('I GOT THIS TOPIC ' + topic)
        # Parse the gerrit host out of self.url.
        host = self.url.split('/')[-1].rstrip('.git')
        changes_with_same_topic = gerrit_util.QueryChanges('%s-review.googlesource.com' % host, [('topic', topic), ('status', 'open')], o_params=['ALL_REVISIONS'])
        for c in changes_with_same_topic:
          if c == change:
            # This change is already in the patch_rev.
            continue
          new_patch_rev = 'refs/changes/%02d/%d/%d' % (c['_number'] % 100, c['_number'], len(c['revisions']))
          print('NEW_PATCH_REV')
          print(new_patch_rev)
          print(patch_rev)
          print(host)
          patch_rev = patch_rev + ';' + new_patch_rev

      print(change_object)
      print('LOOK UP')

    # TODO(rmistry):
    # Change patch_rev to patch_revs. I think that will be cleaner.
    # Still do not know how to get the "skia".
