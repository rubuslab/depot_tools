...
class Mirror(object):
  ...
  def update_bootstrap(self, prune=False, gc_aggressive=False, branch='main'):
    ...
    # to google storage.
    # E.g. `<host_url>-<repo>/<gen_number>/<host_url>-<repo>/`.
    # Check for them here and remove them if found.
    ...
    self.print('running "gsutil -m rsync -r %s %s"' %
               (self.mirror_path, dest_prefix))
    gsutil.call('-m', 'rsync', '-r', self.mirror_path, dest_prefix)
