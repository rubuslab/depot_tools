    ...
    def set_config(f):
        def wrapper(*args):
            ...
            if os.path.exists(os.path.join(args[0].checkout_path, '.git')):
                ...
                if 'diff.ignoresubmodules' not in config or config[
                        'diff.ignoresubmodules'] != ['dirty']:
                    scm.GIT.SetConfig(args[0].checkout_path,
                                      'diff.ignoresubmodules', 'dirty')

                if 'fetch.recursesubmodules' not in config or config[
                        'fetch.recursesubmodules'] != ['off']:
                    scm.GIT.SetConfig(args[0].checkout_path,
                                      'fetch.recursesubmodules', 'off')

                if 'push.recursesubmodules' not in config or config[
    ...
    """
        ...
        if (strp_current_url.rstrip('/') != strp_url.rstrip('/')
                and url != 'git://foo' and scm.GIT.GetConfig(
                    self.checkout_path,
                    'remote.%s.gclient-auto-fix-url' % self.remote) != 'False'):
