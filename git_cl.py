    ...
    if opts.full:
      if opts.dry_run or opts.diff:
        for f in python_diff_files:
          # Get and print diff.
          cmd = [
              yapf_tool, '--diff', '--style', style_args[os.path.abspath(f)], f
          ]
        ...
        for f in python_diff_files:
          ...
          cmd = [yapf_tool, '-i', '--style', style_args[os.path.abspath(f)], f]
          stdout = RunCommand(cmd, error_ok=True, cwd=top_dir)
    else:
      ...
      for f, line_ranges in py_line_diffs.iteritems():
        cmd = [yapf_tool, '--style', style_args[os.path.abspath(f)]]
        for line_range in line_ranges:
          cmd += ['-l', str(line_range[0]) + "-" + str(line_range[1])]
        cmd += [f]
