...
class MockGIT(scm.GIT):
  ...
  def FetchUpstreamTuple(self, _root, _branch):
  ...
  def GetBranchConfig(self, _root, branch, key, default=None):
    return self.GetConfig(_root, 'branch.%s.%s' % (branch, key), default)

  def SetBranchConfig(self, _root, branch, key, value):
    return self.SetConfig(_root, 'branch.%s.%s' % (branch, key), value)

  def CaptureStatus(self, _root, _branch):
    return [('M', 'mod'), ('A', 'add'), ('D', 'delete')]
...
class TestGitCl(unittest.TestCase):
  def setUp(self):
    ...
    self.mockGIT = MockGIT()
    mock.patch('scm.GIT', self.mockGIT).start()
  ...
  @staticmethod
  def _get_gerrit_codereview_server_calls(git_short_host='host',
                                          detect_branch=True):
