...
class TryserverApi(recipe_api.RecipeApi):
  ...
  def initialize(self):
    ...
    if len(changes) == 1:
      ...
      self._gerrit_change_ref = 'refs/changes/%d/%d/%d' % (
          cl.change % 100, cl.change, cl.patchset)

  @property
  def gerrit_change(self):
    """Returns current gerrit change, if there exactly one."""
  ...
  def gerrit_change_repo_url(self):
    ...
    Populated only if gerrit_change is populated.
    """
    return self._gerrit_change_repo_url

  @property
  def gerrit_change_ref(self):
    """Returns current Gerrit change ref, e.g. 'refs/changes/45/12345/1'.

    Populated only if gerrit_change is populated.
    """
    return self._gerrit_change_ref

  @property
  def is_tryserver(self):
    """Returns true iff we have a change to check out."""
    return (self.is_patch_in_git or self.gerrit_change)
