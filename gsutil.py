...
def _run_subprocess(cmd, interactive=False, env=None):
  ...
  # Unified return format for interactive and non-interactive shells.
  SubprocessResponse = namedtuple("SubprocessResponse",
  ...
  if interactive:
    res = subprocess.call(cmd, shell=IS_WINDOWS, env=dict(os.environ, **env))

    return SubprocessResponse(res, None, None)
  ...
  return SubprocessResponse(res.returncode, res.stdout.decode("utf-8"),
