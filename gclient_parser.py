...
class Parser(object):
    ...
    def _object_list_(self):
        """ '[' ws (object:o ws ',' ws -> o)*:hd ws object?:tl ws ']' -> ['object_list', hd + tl] """
        self._expect('[')
        if self.err:
            return
        self._ws_()
    ...
    def _object_(self):
        """ '{' (ws pair:kv ',' -> kv)*:hd ws pair?:tl ws '}' -> ['object', hd + tl] """
        self._expect('{')
        if self.err:
            return
        vs = []
    ...
    def _sq_str_(self):
        """ quote (~(quote|eol) sqchar)*:cs quote -> join('', cs) """
        self._quote_()
        if self.err:
            return
        vs = []
    ...
    def _dq_str_(self):
        """ dquote (~(dquote|eol) dqchar)*:cs dquote -> join('', cs) """
        self._dquote_()
        if self.err:
            return
        vs = []
    ...
    def _name_(self):
        """ (letter|'_'):hd (letter|digit|'_')*:tl -> hd + join('', tl) """
        def group():
            p = self.pos
            def choice_0():
                self._letter_()
