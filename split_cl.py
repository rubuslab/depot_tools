...
def GetCLSizes(files_split_by_owners):
  ...
  for files in files_split_by_owners.values():
    size = sum(f.add + f.rem for f in files)
...
def GetFilesSplitByOwners(files, cl_size):
  ...
  for f in files:
    dir_with_owners = os.path.dirname(f.path)
    ...
    files_split_by_owners.setdefault(dir_with_owners, []).append(f)

  sum_sizes = lambda files: sum(f.add + f.rem for f in files)
  ...
  while changed:
    ...
    for folder, files in reversed(sorted(files_split_by_owners.items())):
      ...
      should_merge = folder != '' and parent != '' and (
...
def PrintClInfo(cl_index, num_cls, directory, files, description, reviewers):
  ...
  lines_added = sum(f.add for f in files)
  lines_removed = sum(f.rem for f in files)
  lines = lines_added + lines_removed
  print('Size: {} (Lines: +{}, -{})'.format(GetCLSize(lines), lines_added,
                                            lines_removed))
...
def SplitCl(description_file, comment_file, cl_size, changelist, cmd_upload,
  ...
  try:
    ...
    files = [
        ChangedFile(add=file_stats[f][0],
                    rem=file_stats[f][1],
                    action=action,
                    path=f)
    ...
    files_split_by_owners = GetFilesSplitByOwners(files, cl_size)
