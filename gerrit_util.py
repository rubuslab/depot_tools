...
def __fixed_rewrite_proxy(self: httplib2.socks.socksocket, header: bytes):
    ...
    if host and endpt:
        ...
        host = host.split(b" ")[1]
        endpt = endpt.split(b" ")
        if self._socksocket__proxy[4] != None and self._socksocket__proxy[
                5] != None:
...
class SSOAuthenticator(Authenticator):
    ...
    def is_applicable(cls) -> bool:
        ...
        skip = scm.GIT.GetConfig(os.getcwd(),
                                 'depot-tools.new-auth-skip-sso') == '1'
        if skip:
            LOGGER.warn(
                'SSOAuthenticator: Skip due to depot-tools.new-auth-skip-sso.')
            return False
        return bool(cls._resolve_sso_binary_path())

    @classmethod
    def _launch_sso_helper(cls) -> str:
        """Launches the git-remote-sso process and extracts the config.
        ...
        timer = threading.Timer(5, _fire_timeout, kwargs={'daemon': True})
        timer.start()
        try:
        ...
        if proc.returncode is not None:
            ...
            with open(tf, encoding='utf-8') as stderr:
                sys.exit(f'SSOAuthenticator: {stderr.read().strip()}')
    ...
    def _get_sso_info(cls) -> SSOInfo:
        with cls._sso_info_lock:
            ...
            if not info:
                ...
                info = cls.SSOInfo(proxy=httplib2.ProxyInfo(
                    httplib2.socks.PROXY_TYPE_HTTP_NO_TUNNEL,
                    proxy_host.encode(), int(proxy_port)),
                                   cookies=http.cookiejar.MozillaCookieJar(
                                       parsed['http.cookiefile']),
                                   headers=headers)
                cls._sso_info = info
            return info
    ...
    def authenticate(self, conn: HttpConn):
        ...
        if (hostname :=
            ...
            parsed = parsed._replace(netloc=hostname_parts[0] +
                                     '.git.corp.google.com')
    ...
    def authenticate(self, conn: HttpConn):
        ...
        if a:
            ...
            if login:
                ...
                conn.req_headers['Authorization'] = f'Basic {secret.decode("utf-8")}'
            else:
...
class GceAuthenticator(Authenticator):
    ...
    def authenticate(self, conn: HttpConn):
        ...
        if not token_dict:
            return
        conn.req_headers['Authorization'] = '%(token_type)s %(access_token)s' % token_dict
...
class LuciContextAuthenticator(Authenticator):
    ...
    def authenticate(self, conn: HttpConn):
        conn.req_headers['Authorization'] = f'Bearer {self._authenticator.get_access_token().token}'
...
def CreateHttpConn(host,
    ...
    authenticator.authenticate(ret)

    if 'Authorization' not in ret.req_headers:
