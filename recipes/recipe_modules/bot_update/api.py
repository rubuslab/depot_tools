...
class BotUpdateApi(recipe_api.RecipeApi):
  ...
  def _get_bot_update_env(self, trace_dir):
    ...
    env = {
        ...
        1800,  # in seconds
        'GIT_TRACE2_EVENT':
        self.m.path.join(trace_dir, 'trace2-event'),
        'GIT_TRACE_CURL':
        self.m.path.join(trace_dir, 'trace-curl'),
        'GIT_TRACE_CURL_NO_DATA':
        1,
        'GIT_TRACE_PACKET':
        self.m.path.join(trace_dir, 'trace-packet'),
        'GIT_BACKENDINFO':
        1,
        'GIT_DAPPER_TRACE':
        1,
  ...
  def ensure_checkout(self,
    ...
    try:
      ...
      step_result = self(name,
                         cmd,
                         env=self._get_bot_update_env(trace_dir),
      ...
      if (step_result.exc_result.was_cancelled
        ...
        zip_path = (self.m.archive.package(trace_dir).archive(
            'compress git traces', self.m.path.join('bot_update_traces.zip')))
