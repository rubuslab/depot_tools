...
class GitSquashBranchTreeTest(git_test_utils.GitRepoReadOnlyTestBase):
    # Empty repo.
    REPO_SCHEMA = """
  """
    ...
    def testGitSquashBranchTreeAll(self):
        assert self._getCountAheadOfUpstream('branchA') == 2
        assert self._getCountAheadOfUpstream('branchB') == 2
        assert self._getCountAheadOfUpstream('branchC') == 2

        self.repo.run(git_squash_branch_tree.main,
                      ['--branch', 'branchA', '--ignore-no-upstream'])

        assert self._getCountAheadOfUpstream('branchA') == 1
        assert self._getCountAheadOfUpstream('branchB') == 1
        assert self._getCountAheadOfUpstream('branchC') == 1

    def testGitSquashBranchTreeSubtree(self):
        assert self._getCountAheadOfUpstream('branchA') == 2
        assert self._getCountAheadOfUpstream('branchB') == 2
        assert self._getCountAheadOfUpstream('branchC') == 2
    ...
    # Returns true if `branch` is exactly `count` ahead of its upstream.
    def _getCountAheadOfUpstream(self, branch):
        parent_of_upstream = '^' + branch + '@{u}'
        output = self.repo.git('rev-list', '--count', branch,
                               parent_of_upstream).stdout
        return int(output)
