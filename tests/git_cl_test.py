...
class CMDOwnersTestCase(CMDTestCaseBase): #unittest.TestCase):
  def setUp(self):
  ...
  def testShowAll(self):
    ...
    self.assertEqual(0, git_cl.main(['owners', '--show-all', 'foo', 'bar']))
    batch_mock.assert_called_once_with(['foo', 'bar']) 
    self.assertEqual(
        '\n'.join([
          'Owners for foo:',
          ' - a@example.com',
