...
class MetricsCollector(object):
  ...
  def print_notice(self, func):
    """A decorator used to print the notice once the program has executed.

    This decorator executes the function and prints the monitoring notice if
    necessary. If |func| raises an exception, this will catch it, print it,
    print the notice and invoke sys.exit() with an appropriate exit code.
    Otherwise it will return the result."""
    # Do this first so we don't have to read, and possibly create a config
    # file.
    if DISABLE_METRICS_COLLECTION:
      return func
    # Don't print a notice if the user has opted in or out or if the user is not
    # a googler.
    if not self.config.is_googler or self.config.opted_in is not None:
      return func
