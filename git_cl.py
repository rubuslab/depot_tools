...
class _GitNumbererState(object):
  ...
  def load(cls, remote_url, remote_ref):
    ...
    if error:
      ...
      # with git-numberer.
      cls._warn('failed to fetch project config for %s: %s',
                remote_url, error)
      return cls(None)
    return cls(cls._is_validator_enabled(remote_ref))
  ...
  def __init__(self, validator_enabled):
    self._validator_enabled = validator_enabled or False
    logging.debug('_GitNumbererState(validator: %s)', self._validator_enabled)
