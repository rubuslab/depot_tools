...
class Dependency(gclient_utils.WorkItem, DependencySettings):
  ...
  def run(self, revision_overrides, command, args, work_queue, options,
    ...
    if run_scm and parsed_url:
      ...
      if command == 'update' and options.gerrit_ref is not None:
        self._used_scm.apply_gerrit_ref(options, file_list)
...
'''  % {'checkout_path': os.path.join(self.root_dir, dep.name),
  ...
  def _EnforceGerritRefs(self):
    ...
    gerrit_refs = {}
    if not self._options.gerrit_patch_refs:
      return gerrit_refs
    for gerrit_patch_ref in self._options.gerrit_patch_refs:
      gerrit_repo, _, gerrit_ref = gerrit_patch_ref.partition('@')
      if not gerrit_repo or not gerrit_ref:
        raise gclient_utils.Error(
            'Both the gerrit repo and the gerrit reference must be specified '
            ' for %s.' % gerrit_patch_ref)
  ...
  def RunOnDeps(self, command, args, ignore_requirements=False, progress=True):
    ...
    if gerrit_refs:
      print('Please fix your script, having invalid --gerrit-ref flags will '
            'soon be considered an error.', file=sys.stderr)
...
def CMDsync(parser, args):
  ...
  parser.add_option('--gerrit-patch-ref', action='append',
                    dest='gerrit_patch_refs', metavar='GERRIT_REF', default=[],
                    help='Patches the given gerrit reference with the format '
                         ...
                         '--gerrit-no-reset-patch-ref to disable this '
                         'behavior.')
  ...
  parser.add_option('--gerrit-no-reset-patch-ref', action='store_false',
