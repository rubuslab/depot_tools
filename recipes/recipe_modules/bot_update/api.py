...
class BotUpdateApi(recipe_api.RecipeApi):
  ...
  def _upload_traces(self):
    with self.m.step.nest('upload git traces') as presentation:
      ...
      zip_path = (self.m.archive.package(self._trace_dir).with_file(
          self._trace_dir.join('trace2-event')).with_file(
              self._trace_dir.join('trace-curl')).with_file(
                  self._trace_dir.join('trace-packet')).archive(
                      'compress traces',
                      self.m.path.join(self._trace_dir, '%s.zip' % id), 'zip'))
      try:
