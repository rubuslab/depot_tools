...
class CMDFormatTestCase(unittest.TestCase):
    ...
    def test_migration_not_ready(self):
        ...
        mock.patch("git_migrate_default_branch.git_common.run", RunMock).start()
        mock.patch(
            "git_migrate_default_branch.git_common.repo_root", return_value="."
        ).start()
        mock.patch(
            "git_migrate_default_branch.scm.GIT.GetConfig",
            return_value="https://chromium.googlesource.com",
        ).start()
        mock.patch(
            "git_migrate_default_branch.gerrit_util.GetProjectHead",
            return_value=None,
        ).start()
    ...
    def test_migration_no_master(self):
        ...
        mock_runs = mock.patch(
            "git_migrate_default_branch.git_common.run", side_effect=RunMock
        ).start()
        mock.patch(
            "git_migrate_default_branch.git_common.repo_root", return_value="."
        ).start()
        ...
        mock.patch(
            "git_migrate_default_branch.git_common.get_branches_info",
        ...
        mock_runs.assert_any_call(
            "branch", "--set-upstream-to", "main", "feature"
        )
        mock_runs.assert_any_call(
            "branch", "--set-upstream-to", "origin/main", "remote_feature"
        )
