...
class ManagedGitWrapperTestCase(BaseGitWrapperTestCase):
    ...
    def testCloneInCog(
        self, mockGetCurrentBranch, mockSetFetchConfig, mockRun, _mockIsCog
    ):
    ...
    def testRevertNone(self):
        ...
        self.assertEqual(file_list, [])
        self.assertEqual(
            scm.revinfo(options, self.args, None),
    ...
    def testUpdateMerge(self):
        ...
        scm.update(options, (), file_list)
        self.assertEqual(
            file_list, [join(self.base_path, x) for x in ["a", "b", "c"]]
        )
        ...
        self.assertEqual(
            scm._Capture(["rev-parse", parent + "2"]),
    ...
    def testUpdateRebase(self):
        ...
        parent = "HEAD^" if sys.platform != "win32" else "HEAD^^"
        self.assertEqual(
            scm._Capture(["rev-parse", parent + "1"]),
            scm._Capture(["rev-parse", "origin/main"]),
        )
        sys.stdout.close()
...
class UnmanagedGitWrapperTestCase(BaseGitWrapperTestCase):
    ...
    def testUpdateCloneOnCommit(self):
        ...
        self.base_path = join(self.root_dir, self.relpath)
        url_with_commit_ref = (
            origin_root_dir + "@a7142dc9f0009350b96a11f372b6ea658592aa95"
        )
