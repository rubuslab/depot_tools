...
def solutions_printer(solutions):
    ...
    for solution in solutions:
        ...
        if "managed" in solution:
            print(
                "  Managed mode is %s"
                % ("ON" if solution["managed"] else "OFF")
            )
...
def ensure_checkout(
    ...
    for sln in solutions:
        sln["deps_file"] = sln.get("deps_file", "DEPS").replace(
            ".DEPS.git", "DEPS"
        )
    gclient_configure(
        solutions, target_os, target_os_only, target_cpu, git_cache_dir
    ...
    for revision in expanded_revisions:
        ...
        if len(split_revision) == 1:
            ...
            if parsed_root.scheme in ["http", "https"]:
                ...
                normalized_root = (
                    "https://" + parsed_root.netloc + parsed_root.path
                )
                if not normalized_root.endswith(".git"):
                    normalized_root += ".git"
            elif parsed_root.scheme:
                print(
                    "WARNING: Unrecognized scheme %s, ignoring"
                    % parsed_root.scheme
                )
                continue
            else:
...
def parse_args():
    ...
    if options.spec_path:
        if options.specs:
            parse.error(
                "At most one of --spec-path and --specs may be specified."
            )
...
def checkout(options, git_slns, specs, revisions, step_text):
    ...
    cache_epoch_marker_path = os.path.join(
        options.git_cache_dir, ".cache_epoch"
    )
    ...
    print(
        "git_cache epoch: {}".format(datetime.fromtimestamp(int(cache_epoch)))
    )
