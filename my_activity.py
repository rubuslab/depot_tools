...
class MyActivity(object):
  ...
  def gerrit_changes_over_rest(self, instance, filters):
    ...
    try:
      ...
      # errors here.
      changes = list(gerrit_util.GenerateAllChanges(instance['url'], req,
  ...
  def monorail_validate_issue_modified(self, issue):
    ...
    url = ('https://monorail-prod.appspot.com/_ah/api/monorail/v1/projects'
           '/%s/issues/%s/comments') % (project, issue_id)
    ...
    for item in content.get('items', []):
      comment_published = dateutil.parser.parse(item['published'])
