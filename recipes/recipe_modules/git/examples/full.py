...
def RunSteps(api):
    ...
    api.git.count_objects(
        name="count-objects",
        raise_on_failure=api.properties.get(
            "count_objects_can_fail_build", False
        ),
    ...
    api.git(
        "status", name="git status cannot_fail_build", raise_on_failure=False
    )
    ...
    api.git.bundle_create(api.path["start_dir"].join("all.bundle"))


def GenTests(api):
    yield api.test("basic")
    yield api.test("basic_tags") + api.properties(tags=True)
    yield api.test("basic_ref") + api.buildbucket.ci_build(
        git_ref="refs/foo/bar"
    )
    ...
    yield api.test("basic_file_name") + api.properties(
        checkout_file_name="DEPS"
    )
    ...
    yield api.test("remote_not_origin") + api.properties(
        remote_name="not_origin"
    )
