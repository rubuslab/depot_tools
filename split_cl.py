...
def MergeSmallCls(files_split_by_owners, min_cl_size=0, max_cl_size=100000):
  needs_merge = True
  while needs_merge:
    needs_merge = False
    ...
    for folder, files in files_split_by_owners.iteritems():
      ...
      if any(
          sum(sum(f.DiffStat())
              for f in files) > max_cl_size
          for files in new_split.itervalues()):
        break

      files_split_by_owners = new_split
  print(files_split_by_owners.keys())
  return files_split_by_owners
...
def SplitCl(description_file, comment_file, changelist, cmd_upload, dry_run,
  ...
  try:
    ...
    files_split_by_owners = MergeSmallCls(files_split_by_owners, 400)
