...
class Changelist(object):
  ...
  def _RunGitPushWithTraces(
      self, refspec, refspec_opts, git_push_metadata):
    ...
    try:
      ...
      raise GitPushError('Failed to create a change. Please examine output above '
  ...
  def CMDUploadChange(
    ...
    try:
      ...
      if is_retry is False and remote_branch == DEFAULT_OLD_BRANCH and \
          gerrit_util.GetProjectHead(
              self._gerrit_host, self._GetGerritProject()) == 'refs/heads/main':
        print(
            "WARNING: Detected HEAD change in upstream, fetching remote state")
        remote, remote_branch = self.GetRemoteBranch()
        RunGit(['fetch', remote])
        options.edit_description = False
        options.title = title
        options.force = True
        return self.CMDUploadChange(
            options, git_diff_args, custom_cl_base, change_desc, True)
