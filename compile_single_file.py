...
def main():
  ...
  src_relpath = os.path.relpath(options.file_path,
                                os.path.join(src_dir, options.build_dir))
  ...
  command = [
      ninja_exec,
      '-C', os.path.join(src_dir, options.build_dir),
      '%s%s' % (src_relpath, carets)
  ]
  # |shell| should be set to True on Windows to avoid dropping the caret
  # characters.
