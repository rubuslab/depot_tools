...
"""Creates a new infra_superpoject gclient checkout based on an existing infra
  ...
  run: `python path/to/depot_tools/infra_to_superproject.py --destination=~/cr2`
"""
...
def main(argv):
  ...
  parser.add_argument('--destination',
                      default='~/cr2',
                      help="Location of the new directory.")
  options = parser.parse_args(argv)

  # In case there is '~' in the destination string
  options.destination = os.path.expanduser(options.destination)
