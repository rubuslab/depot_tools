...
class GitConfigStateBase(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def set_config_multi(self, key: str, value: str, *, value_pattern: Optional[str],
                         scope: GitConfigScope):
...
class GitConfigUneditableScope(ValueError):
    ...
    def __init__(self, scope: str) -> None:
        super().__init__(
            f'Cannot edit git config in scope {scope!r}.')
...
class GitConfigUnknownScope(ValueError):
    ...
    def __init__(self, scope: str) -> None:
        super().__init__(
            f'Unknown git config scope {scope!r}.')
...
class GitConfigStateReal(GitConfigStateBase):
    ...
    def unset_config(self, key: str, *, scope: GitConfigScope,
        ...
        try:
            ...
            if cpe.returncode == 5:
                if 'multiple values' in cpe.stderr:
