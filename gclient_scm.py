...
class GitWrapper(SCMWrapper):
  ...
  def _Clone(self, revision, url, options):
    ...
    if in_cog_workspace:
      ...
      # Ideally depth would be 1 here, but that doesn't work well with tools
      # that look for tags in commit messages that might not be present at HEAD.
      # e.g. lastchange.py
      revision = self._AutoFetchRef(options, revision, depth=5)
      remote_ref = scm.GIT.RefToRemoteRef(revision, self.remote)
