...
def IsRustSupported():
  # TODO(lukasza): Support discovering a custom `rust_bin_dir` when the official
  # tool chain is missing - when `use_unverified_rust_toolchain = true`.`
  return sys.platform.startswith('linux')


def FindRustfmtToolInChromiumTree():
  """Return a path to the rustfmt executable, or die trying."""
  if not IsRustSupported():
    raise NotFoundError('No Rust support in this environment.\n')
  ...
  # DO NOT SUBMIT: Once https://crrev.com/c/2162136 lands, make sure that the
  # code below uses the same path (same as `rust_prefix` in
  # `//build/config/rust.gni` in that CL).
  #
  # TODO(lukasza): Deduplicate the is-Rust-supported and find-Rust-binaries code
  # by somehow sharing the code with //build/config/rust.gni
  tool_path = os.path.join(chromium_src_path,
                           'third_party',
