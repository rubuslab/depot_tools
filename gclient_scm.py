...
class GitWrapper(SCMWrapper):
  ...
  def update(self, options, args, file_list):
    ...
    if (current_url.rstrip('/') != url.rstrip('/') and url != 'git://foo' and
        subprocess2.capture(
            scm.Git(['config', 'remote.%s.gclient-auto-fix-url' % self.remote]),
  ...
  def _Capture(self, args, **kwargs):
    ...
    ret = subprocess2.check_output(
        scm.Git(args), env=env, **kwargs).decode('utf-8')
    if strip:
      ret = ret.strip()
    self.Print('Finished running:', ' '.join(scm.Git(args)))
    return ret
  ...
  def _Run(self, args, options, **kwargs):
    ...
    env = scm.GIT.ApplyEnvVars(kwargs)
    gclient_utils.CheckCallAndFilter(scm.Git(args), env=env, **kwargs)
