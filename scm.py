...
class GIT(object):
    ...
    def _load_config(cwd) -> Dict[str, List[str]]:
        cached: Optional[Dict[str,
                              List[str]]] = GIT._CONFIG_CACHE.get(cwd, None)
        if cached is None:
            cached = {}
            try:
    ...
    def ApplyEnvVars(kwargs):
        ...
        #
        # By default, git will use an interactive terminal prompt when
        # a username/ password is needed.  That shouldn't happen in the chromium
