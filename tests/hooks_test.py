...
PRECOMMIT_PATH = os.path.join(ROOT_DIR, 'hooks', 'pre-commit.py')
...
class HooksTest(unittest.TestCase):
    ...
    def populate_child(self):
        git.run('init', cwd=self.child_repo)
        self.write(self.child_repo, 'baz', 'baz')
        git.run('add', '.', cwd=self.child_repo)
        git.run('commit', '-m', 'init', cwd=self.child_repo)
    ...
    def populate(self):
        ...
        os.mkdir(os.path.join(self.repo, 'dep_a'))
