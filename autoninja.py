            ...
            if match:
                ...
                if raw_import_path[:2] == "//":
                    import_path = os.path.normpath(
                        os.path.join(output_dir, "..", "..", raw_import_path[2:])
                    )
                else:
                    import_path = os.path.normpath(
...
def main(args):
    ...
    if os.path.exists(os.path.join(output_dir, "args.gn")):
        for line in _gn_lines(output_dir, os.path.join(output_dir, "args.gn")):
            ...
            if re.search(r"(^|\s)(use_goma)\s*=\s*true($|\s)", line_without_comment):
                use_goma = True
                continue
            if re.search(
                r"(^|\s)(use_remoteexec)\s*=\s*true($|\s)", line_without_comment
            ):
                use_remoteexec = True
                continue
            if re.search(r"(^|\s)(use_siso)\s*=\s*true($|\s)", line_without_comment):
                use_siso = True
                continue
        ...
        if use_siso:
            ...
            if use_goma:
                ...
                print("Do not use use_siso=true and use_goma=true", file=sys.stderr)
                return 1
            if use_remoteexec:
