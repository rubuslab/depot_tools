...
def CheckChangedLUCIConfigs(input_api, output_api):
  ...
  """
    ...
    # authentication
    try:
        acc_tkn = auth.Authenticator().get_id_token()
    ...
    def format_config_set(cs):
        ...
        return {
            ...
            'location': cs.get('url'),
            'revision': {
                'id': cs.get('revision').get('id'),
                'url': cs.get('revision').get('url'),
                'timestamp': cs.get('revision').get('timestamp'),
                'committer_email': cs.get('revision').get('committerEmail')
            }
        }
