...
class PresubmitApi(recipe_api.RecipeApi):
  ...
  def relative_root(self, is_try):
    self._relative_root = self.m.gclient.get_gerrit_patch_root().rstrip('/') if is_try else self.m.gclient.c.solutions[0].name.rstrip('/')
  ...
  def prepare(self, root_solution_revision=None):
    ...
    if not self._relative_root :
      self._relative_root = is_try

    abs_root = self.m.context.cwd.join(self.relative_root)
    if is_try:
  ...
  def execute(self, bot_update_step, skip_owners=False, run_all = False):
    ...
    got_revision_properties = self.m.bot_update.get_project_revision_properties(
        ...
        self._relative_root.replace(self.m.path.sep, '/'), self.m.gclient.c)
