...
Target is the file or directory intended to be uploaded to Google Storage.
If that single target is a directory, it will be compressed and uploaded as a
tar.gz file.
...
def create_archive(object_name, dirs):
    # tarfile name cannot have a forward slash or else an error will be
    # thrown
    tarname = os.path.basename(object_name) + '.tar.gz'
...
def upload_to_google_storage(file, base_url, object_name, gsutil, force, gzip,
                             dry_run):
...
def construct_deps_blob(bucket, object_name, file):
...
def main():
    ...
    parser.add_option('-o',
                      '--object-name',
                      help='Object name of uploaded tar file.')
    parser.add_option('-d',
                      '--dry-run',
                      action='store_true',
    ...
    if len(input_filenames) > 1 or (len(input_filenames) == 1
        ...
        if not validate_archive_dirs(input_filenames):
            parser.error(
                ...
                input_filenames)
            return 1
        file = create_archive(options.object_name, input_filenames)
    else:
    ...
    upload_to_google_storage(file, base_url, options.object_name, gsutil,
                             options.force, options.gzip, options.dry_run)
    print(
        json.dumps(construct_deps_blob(options.bucket, options.object_name,
                                       file),
                   indent=2))
