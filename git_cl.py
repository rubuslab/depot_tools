...
def CreateBranchForIssue(issue):
  ...
  cl = Changelist(issue=issue)
  try:
    # TODO: add `("repo", repo_name)` to ensure the issue is in current repo.
    # currently, an issue number existing but in another repo would lead to
    # 300 seconds of 404 errors then timeout.
    changes = gerrit_util.QueryChanges(cl.GetGerritHost(),
                                       [('change', '%s' % issue)])
    if not changes:
      # No issue with this issue number
      return None
  except gerrit_util.GerritError:
    # Connection problem, timeout...
    return None
  try:
    detail = cl._GetChangeDetail(['CURRENT_REVISION', 'CURRENT_COMMIT'])
  except GerritChangeNotExists:
