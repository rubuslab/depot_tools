...
def main(args):
  ...
  gclient_src_root_path = os.path.join(gclient_root_path, 'src') if gclient_root_path else None
  for base_path in set([primary_solution_path, gclient_root_path, gclient_src_root_path]):
    if not base_path:
      continue
