...
def get_target_branch_and_revision(solution_name, git_url, revisions):
  ...
  if branch.starts('refs/heads/'):
    branch = 'refs/remotes/origin/' + branch[len('refs/heads/'):]
  elif branch.starts('refs/branch-heads/'):
  ...
  while True:
    try:
      ...
      if not path.isdir(sln_dir):
        ...
        # HEAD after cloning.
        git('checkout', branch, '--detach', cwd=sln_dir)
      ...
      # Note that '--' argument is needed to ensure that git treats
