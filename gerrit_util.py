...
class CookiesAuthenticator(Authenticator):
    ...
    def _get_netrc_from_path(cls, path):
        try:
            ...
            print(
                "WARNING: Could not read netrc file %s" % path, file=sys.stderr
            )
            return netrc.netrc(os.devnull)
        except netrc.NetrcParseError as e:
            print(
                "ERROR: Cannot use netrc file %s due to a parsing error: %s"
                % (path, e),
                file=sys.stderr,
            )
            return netrc.netrc(os.devnull)
...
def CreateHttpConn(
    host, path, reqtype="GET", headers=None, body=None, timeout=300
):
