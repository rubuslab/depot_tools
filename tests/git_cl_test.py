...
class CMDTryResultsTestCase(unittest.TestCase):
  _STATUSES = [
  ...
  def setUp(self):
    super(CMDTryResultsTestCase, self).setUp()
    ...
    mock.patch('git_cl.Changelist._GetChangeDetail',
               _constantFn(self._CHANGE_DETAIL)).start()
    mock.patch('git_cl._call_buildbucket').start()
...
class CMDUploadTestCase(unittest.TestCase):
  ...
  def setUp(self):
    ...
    mock.patch('git_cl.Changelist._GetChangeDetail',
               _constantFn(self._CHANGE_DETAIL)).start()
    mock.patch('git_common.is_dirty_git_tree', _constantFn(False)).start()
    mock.patch('git_cl.fetch_try_jobs').start()
    git_cl.fetch_try_jobs.return_value = self._DEFAULT_TRY_JOBS
    self.addCleanup(mock.patch.stopall)

  @mock.patch('git_cl._trigger_try_jobs')
  @mock.patch('git_cl.Changelist.CMDUpload', _constantFn(0))
  def testUploadRetryFailed(self, mockTriggerTryJobs):
    ...
    git_cl.fetch_try_jobs.assert_called_with(
