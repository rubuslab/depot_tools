...
class GitWrapper(SCMWrapper):
  ...
  def apply_patch_ref(self, patch_repo, patch_rev, target_rev, options,
    ...
    if options.download_topics:
      # TODO: This does not authenticate into the Gerrit host. It makes
      # annonymous calls to get information about the change. Till support for
      # authentication is added this will not work for private hosts and
      # changes..
    ...
    for patch_rev in patch_revs_to_process:
      ...
      if not options.rebase_patch_ref:
        ...
        try:
          if scm.GIT.IsAncestor(self.checkout_path, patch_rev, target_rev):
            ...
            # If a change was uploaded on top of another change, which hasalready
