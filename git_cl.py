...
def ConfigureGitAuth() -> None:
    ...
    if c2.mode == c.mode:
        logging.debug(
            'Local user wants same mode %s as global; skipping local repo auth config',
            c2.mode)
        # TODO(ayatane): Clean up any previous local repo config. Remove after a few weeks
        c2.mode = GitAuthMode.OLD_AUTH
        c2.apply(cwd)
        return
