...
def GetUnitTestsRecursively(input_api,
                            ...
                            run_on_python2=True,
                            run_on_python3=True):
  """Gets all files in the directory tree (git repo) that match the allowlist.

  Restricts itself to only find files within the Change's source repo, not
  dependencies.
  """
  def check(filename):
    return (any(input_api.re.match(f, filename) for f in allowlist)
            and not any(input_api.re.match(f, filename) for f in blocklist))
...
def GetPylint(input_api,
  ...
  """
  allow_list = tuple(allow_list or (r'.*\.py$', ))
  block_list = tuple(block_list or input_api.DEFAULT_BLOCK_LIST)
  ...
  src_filter = lambda x: input_api.FilterSourceFile(x, map(
      rel_path, allow_list), map(rel_path, block_list))
