...
class _GerritChangelistImpl(_ChangelistCodereviewBase):
  ...
  def _CleanUpOldTraces(self):
    ...
    traces = [f for f in os.listdir(TRACES_DIR) if os.path.isfile(f)]
    traces_to_delete = traces[:-MAX_TRACES]
    print(TRACES_DIR)
    print(os.listdir(TRACES_DIR))
    print(traces)
    print(traces_to_delete)
  ...
  def _WriteGitPushTraces(self, traces_dir, git_push_metadata):
    ...
    with open(traces_metadata, 'w') as f:
      f.write(TRACES_METADATA_FORMAT % git_push_metadata)
    ...
    print(TRACES_MESSAGE % (traces_zip, git_info_zip))
  ...
  def _RunGitPushWithTraces(
    ...
    try:
      ...
      git_push_metadata['date'] = datetime.datetime.now().strftime('%c')
