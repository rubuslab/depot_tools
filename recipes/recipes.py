    ...
    try:
        if (version := recipes_cfg["api_version"]) != 2:
            raise MalformedRecipesCfg(
                f"unknown version {version}", recipes_cfg_path
            )
...
def checkout_engine(engine_path, repo_root, recipes_cfg_path):
    ...
    if not engine_path:
        ...
        engine_path = os.path.join(
            recipes_path, ".recipe_deps", "recipe_engine"
        )
        ...
        try:
            ...
            _git_check_call(
                ["reset", "-q", "--hard", revision], cwd=engine_path
            )
...
def main():
    ...
    if recipes_cfg_path:
        ...
        recipes_cfg_path = os.path.join(
            repo_root, "infra", "config", "recipes.cfg"
        )
