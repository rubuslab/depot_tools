...
def get_actual_path(path):
  """Returns actual path. noop if platform != cygwin."""
  if sys.platform != 'cygwin':
    return path

  return subprocess.check_output(['cygpath', '-w',
                                  path]).strip().decode('utf-8', 'replace')
...
def cipd_bin_setup():
  ...
  if not (os.path.exists(CIPD_BIN) and os.path.exists(GSUTIL_SHIM_PATH)):
    cmd = [
        get_actual_path(CIPD_COMMAND), 'ensure', '-log-level', 'warning',
        '-ensure-file',
        get_actual_path(CIPD_MANIFEST), '-root',
        get_actual_path(CIPD_BIN)
    ]
