...
def _ComputeDiffLineRanges(files, upstream_commit):
  """Gets the changed line ranges for each file between upstream_commit.
...
def _FindYapfConfigFile(fpath, yapf_config_cache, top_dir=None, default_style=None):
...
def CMDformat(parser, args):
  ...
  if opts.python and python_diff_files:
    ...
    for f in python_diff_files:
      # Find the yapf style config for the current file, defaults to depot tools default.
      yapf_config = _FindYapfConfigFile(
          os.path.abspath(f), yapf_configs, top_dir, chromium_default_yapf_style)
      ...
      if not opts.full:
        ...
        for line_range in py_line_diffs[f]:
          diff_start = line_range[0]
          diff_end = line_range[0] + line_range[1] - 1
