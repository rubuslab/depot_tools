...
# The GFE limit is 32 MiB. Give 2KiB buffer room.
_REQUEST_BODY_SIZE_LIMIT = 32 * 1024 * 1024 - 2 * 1024
...
_UNCOMPRESSED_CONFIG_SIZE_LIMIT_PER_REQUEST = 5 * 32 * 1024 * 1024
...
def CheckChangedLUCIConfigs(input_api, output_api):
  ...
  def request(endpoint, body=None):
    ...
    if body is not None:
      ...
      if len(req.data) > _REQUEST_BODY_SIZE_LIMIT:
        req.data = zlib.compress(req.data)
  ...
  for cs, files in cs_to_files.items():
    ...
    # size limit.
    for f in files:
      if len(f['content']) > _UNCOMPRESSED_CONFIG_SIZE_LIMIT_PER_REQUEST:
        return [
            output_api.PresubmitError(
                ('File %s grows too large, it is now ~%.2f MiB. '
                 'The limit is %.2f MiB') %
                (f['path'], len(f['content']) /
                 (1024 * 1024), _UNCOMPRESSED_CONFIG_SIZE_LIMIT_PER_REQUEST /
                 (1024 * 1024)))
        ]
    ...
    for idx, f in enumerate(files):
      content = f['content']
      if curSize + len(content) > _UNCOMPRESSED_CONFIG_SIZE_LIMIT_PER_REQUEST:
        try:
