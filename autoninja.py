...
def _is_corp_machine_using_external_account():
    ...
    return bool(account and not account.endswith("@google.com"))
...
def main(args):
    ...
    if use_remoteexec or use_siso:
        if _is_corp_machine_using_external_account():
            print(
                "You shouldn't use a non-@google.com account (%s and/or %s) on"
                " a corp machine.\n"
                "Please login via `gcloud auth login` or `gcloud auth"
                " application-default login` with your @google.com account"
