...
class Changelist(object):
  ...
  def _CMDUploadChange(self, options, git_diff_args, custom_cl_base,
    ...
    if options.squash:
      ...
      if self.GetIssue():
        print("self.upstream_parent before fetch:", self.upstream_parent)
        self.upstream_parent = self._PatchExternalChanges()
        print("self.upstream_parent after fetch:", self.upstream_parent)
      ...
      remote, upstream_branch = self.FetchUpstreamTuple(self.GetBranch())
      parent = self.upstream_parent or self._ComputeParent(
  ...
  def _PatchExternalChanges(self):
    ...
    Returns the commit hash of the latest upstream patchset's parent if fetched.
    ...
    with gclient_utils.temporary_file() as diff_tempfile:
      ...
      with gclient_utils.temporary_file() as patch_tempfile:
        ...
        # branch = scm.GIT.GetBranchRef(settings.GetRoot()) maybe this gives head detached?
