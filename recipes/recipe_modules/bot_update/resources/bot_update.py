...
# Regular expression to match patch failure message from gclient sync.
PATCH_FAILURE_RE = re.compile("Failed to apply '.*' @ '.*' to '.*' at '.*'")
...
def gclient_sync(
  ...
  try:
    ...
    if PATCH_FAILURE_RE.search(e.output):
