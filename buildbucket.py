...
def main(argv):
  ...
  retry_parser.add_argument(
    '--client-operation-id',
    help='The ID of the operation generated by caller. Buildbucket will only '
         'schedule one build for multiple requests with the same client and '
         'build IDs issued within a few seconds from each other. All such '
         'requests will receive a successful response with scheduled build ID.',
  )
  retry_parser.add_argument(
    '--lease-expiration-secs',
    type=float,
    help='When the lease will expire in seconds from current time. Not '
         'supported for LUCI/swarmbucket builds.',
  )
  ...
  if args.command == 'get':
    ...
    body = {}
    if args.client_operation_id is not None:
      body['client_operation_id'] = str(args.client_operation_id)
    if args.lease_expiration_secs is not None:
      timestamp_usec = int((time.time() + args.lease_expiration_secs) * 1000000)
      body['lease_expiration_ts'] = str(timestamp_usec)
