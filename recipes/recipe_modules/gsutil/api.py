...
class GSUtilApi(recipe_api.RecipeApi):
  ...
  def upload(self, source, bucket, dest, args=None, link_name='gsutil.upload',
    ...
    if link_name:
      console_url = '-r' in args or '--recursive' in args
  ...
  @classmethod
  def _http_url(cls, bucket, dest, unauthenticated_url=False, console_url=False):
    if console_url:
      base = 'https://console.cloud.google.com/storage/browser/%s/%s'
    elif unauthenticated_url:
