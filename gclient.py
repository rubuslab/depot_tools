...
"""
    ...
    def RunOnDeps(
        ...
        if command == "update":
            ...
            is_submoduled = any(
                map(
                    lambda d: d.git_dependencies_state
...
class Flattener(object):
    ...
    def _flatten_dep(self, dep):
        ...
        for key, value in dep._vars.items():
            ...
            assert (
                key not in self._vars or self._vars[key][1] == value
...
def CMDgitmodules(parser, args):
    ...
    print(
        ".gitmodules and gitlinks updated. Please check git diff and "
        "commit changes."
    )
...
@subcommand.epilog(
    """For more information on what metrics are we collecting and
