...
def _call_buildbucket(http, buildbucket_host, method, request):
    ...
    url = "https://%s/prpc/buildbucket.v2.Builds/%s" % (
        buildbucket_host,
        method,
    )
    ...
    while True:
        response, content = http.request(
            url, "POST", body=request, headers=headers
        )
...
def _trigger_tryjobs(changelist, jobs, options, patchset):
    ...
    batch_response = _call_buildbucket(
        http, DEFAULT_BUILDBUCKET_HOST, "Batch", batch_request
    )
...
def _fetch_tryjobs(changelist, buildbucket_host, patchset=None):
    ...
    if authenticator.has_cached_credentials():
        ...
        print(
            "Warning: Some results might be missing because %s"
            %
    ...
    response = _call_buildbucket(
        http, buildbucket_host, "SearchBuilds", request
    )
...
class Settings(object):
    ...
    def IsStatusCommitOrderByDate(self):
        if self.is_status_commit_order_by_date is None:
            result = RunGit(
                ["config", "--bool", "cl.date-order"], error_ok=True
            ).strip()
...
class Changelist(object):
    ...
    def EnsureAuthenticated(self, force, refresh=None):
        ...
        if gerrit_auth and git_auth:
            ...
            if not force:
                confirm_or_exit(
                    "If you know what you are doing", action="continue"
                )
            return
    ...
    def EnsureCanUploadPatchset(self, force):
        ...
        if self.GetIssueOwner() == cookies_user:
            return
        logging.debug(
            "change %s owner is %s, cookies user is %s",
            self.GetIssue(),
            self.GetIssueOwner(),
            cookies_user,
        )
    ...
    def GetCommentsSummary(self, readable=True):
        ...
        for path, robot_comments in robot_file_comments.items():
            ...
            line_comments.extend(
                [
                    c
                    for c in robot_comments
                    if c["patch_set"] == latest_patch_set
                ]
            )
        ...
        for path, line_comments in file_comments.items():
            for comment in line_comments:
                tag = comment.get("tag", "")
                if (
                    tag.startswith("autogenerated")
                    and "robot_id" not in comment
                ):
                    continue
    ...
    def CMDPatchWithParsedIssue(
        self, parsed_issue_arg, nocommit, force, newbranch
    ):
    ...
    def _UpdateWithExternalChanges(self):
        ...
        external_parent = self._GetChangeCommit(revision=external_ps)[
            "parents"
        ][0]
...
def LoadCodereviewSettingsFromFile(fileobj):
    ...
    SetProperty(
        "cpplint-ignore-regex", "LINT_IGNORE_REGEX", unset_error_ok=True
    )
    SetProperty(
        "run-post-upload-hook", "RUN_POST_UPLOAD_HOOK", unset_error_ok=True
    )
    SetProperty(
        ...
        if self._all_hosts is None:
            ...
            self._all_hosts = [
                ...
                for h, u, s in itertools.chain(
                    (
                        (h, u, ".netrc")
                        for h, (u, _, _) in a.netrc.hosts.items()
                    ),
                    (
                        (h, u, ".gitcookies")
...
def CMDbaseurl(parser, args):
    ...
    if not args:
        ...
        return RunGit(
            ["config", "branch.%s.base-url" % branch], error_ok=False
        ).strip()

    print("Setting base-url to %s" % args[0])
    return RunGit(
        ["config", "branch.%s.base-url" % branch, args[0]], error_ok=False
    ).strip()
...
def CMDissue(parser, args):
    ...
    if options.reverse:
        ...
        git_config = {}
        for config in RunGit(
            ["config", "--get-regexp", r"branch\..*issue"]
        ).splitlines():
...
def CMDcomments(parser, args):
    ...
    summary = sorted(
        cl.GetCommentsSummary(readable=options.readable), key=lambda c: c.date
    )
...
def CMDupload(parser, args):
    ...
    parser.add_option(
        "--message", "-m", dest="message", help="message for patchset"
    )
    parser.add_option(
        "-b",
        "--bug",
    ...
    disable_dogfood_stacked_changes = (
        os.environ.get(DOGFOOD_STACKED_CHANGES_VAR) == "0"
    )
    ...
    if (
        options.squash
        and not dogfood_stacked_changes
...
def CMDpatch(parser, args):
    ...
    if options.reapply:
        ...
        return cl.CMDPatchWithParsedIssue(
            target_issue_arg, options.nocommit, options.force, False
        )
    ...
    return cl.CMDPatchWithParsedIssue(
        target_issue_arg, options.nocommit, options.force, options.newbranch
    )
...
def CMDformat(parser, args):
    ...
    return_value = _RunClangFormatDiff(
        opts, clang_diff_files, top_dir, upstream_commit
    )
    ...
    if opts.use_swift_format:
        ...
        swift_format_return_value = _RunSwiftFormat(
            opts, swift_diff_files, top_dir, upstream_commit
        )
    ...
    if python_diff_files and not py_explicitly_disabled:
        ...
        if not opts.full and filtered_py_files:
            py_line_diffs = _ComputeDiffLineRanges(
                filtered_py_files, upstream_commit
            )

        yapfignore_patterns = _GetYapfIgnorePatterns(top_dir)
        filtered_py_files = _FilterYapfIgnoredFiles(
            filtered_py_files, yapfignore_patterns
        )
        ...
        for f in filtered_py_files:
            ...
            if opts.diff or opts.dry_run:
                ...
                cmd += ["-i"]
                RunCommand(
                    cmd, cwd=top_dir, shell=sys.platform.startswith("win32")
                )
