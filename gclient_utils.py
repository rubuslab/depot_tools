...
def CheckCallAndFilter(args, print_stdout=False, filter_fn=None,
  ...
  for _ in range(RETRY_MAX + 1):
    ...
    if print_stdout:
      sys.stdout.flush()
      output_buffer = getattr(sys.stdout, 'buffer', sys.stdout)
    else:
    ...
    try:
      ...
      while True:
        ...
        if is_newline and filter_fn:
          filter_fn(output_buffer.getvalue().decode('utf-8'))
          output_buffer.seek(0)
          output_buffer.truncate()
        elif is_newline and print_stdout:
          output_buffer.write(in_byte)
          sys.stdout.flush()
        else:
          output_buffer.write(in_byte)
