...
class FilePatchBase(object):
  ...
  def filename_after_patchlevel(self):
    ...
    return self.apply_patchlevel(self.filename, self.patchlevel-1)

  def apply_patchlevel(self, string, patchlevel=None):
...
class FilePatchDiff(FilePatchBase):
  ...
  def _verify_git_header(self):
    ...
    while lines:
      ...
      if match.group(1).startswith('a/') and match.group(2).startswith('b/'):
        self.patchlevel = 1
      old = self.apply_patchlevel(match.group(1))
      new = self.apply_patchlevel(match.group(2))
  ...
  def _verify_git_header_process_line(self, lines, line, last_line):
    ...
    if match:
      ...
      if match.group(1) == '/dev/null':
        self.is_new = True
      elif self.apply_patchlevel(match.group(1)) != old:
    ...
    if match:
      ...
      if '/dev/null' == match.group(1):
        self.is_delete = True
      elif self.filename_utf8 != self.apply_patchlevel(match.group(1)):
        self._fail(
  ...
  def _verify_svn_header_process_line(self, lines, line, last_line):
    ...
    if match:
      ...
      if match.group(1) == '/dev/null':
        self.is_new = True
      elif self.apply_patchlevel(match.group(1)) != self.filename_utf8:
