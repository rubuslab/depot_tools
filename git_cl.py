...
def _RunSwiftFmt(opts, swift_diff_files, top_dir, upstream_commit):
  ...
  swift_format_tool = os.path.join(depot_tools_path, 'swift-format')
...
def CMDformat(parser, args):
  ...
  parser.add_option(
      ...
      action='store_true',
      default=False,
      help='Enables formatting of Swift files using swift-format.')
  parser.add_option(
      '--no-swift',
      dest='use_swift_fmt',
      action='store_false',
  ...
  if opts.use_swift_fmt:
    if sys.platform != 'darwin':
      DieWithError('swift-format is only supported on macOS.')
      
    swift_fmt_return_value = _RunSwiftFmt(opts, swift_diff_files, top_dir,
                                        upstream_commit)
    if swift_fmt_return_value == 2:
      return_value = 2
