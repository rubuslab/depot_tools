...
class BaseTest(fake_repos.FakeReposTestBase):
  ...
  def _check_delete(self, co):
    """Makes sure file deletions are handled correctly."""
    co.prepare(None)
    patchset = patch.PatchSet([
        patch.FilePatchDelete(
            'chromeos/views/DOMui_menu_widget.h', False),
    ])
    for p in patchset:
      p.decrement_patchlevel = True
      p.patchlevel = 1
    co.apply_patch(patchset)
    # Make sure chromeos/views/DOMui_menu_widget.h is deleted and
    # chromeos/views/webui_menu_widget.h is correctly created.
  ...
  def _check_delete_patchlevel(self, co):
    """Makes sure file deletions are handled correctly when patchlevel > 1."""
    co.prepare(None)
    patchset = patch.PatchSet([
        patch.FilePatchDelete(
