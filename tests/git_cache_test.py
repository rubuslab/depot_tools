...
class GitCacheTest(unittest.TestCase):
  def setUp(self):
    ...
    mock.patch('os.environ', {
        'GIT_CONFIG_SYSTEM': os.path.join(self.cache_dir, '.gitconfig'),
    }).start()
    ...
    self.git([
        'config', '--file',
        os.path.join(self.cache_dir, '.gitconfig'), 'user.name', '"Your name"'
    ])
    self.git([
        'config', '--file',
        os.path.join(self.cache_dir, '.gitconfig'), 'user.email',
        '"email@xample.com"'
    ])
