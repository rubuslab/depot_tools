...
def CMDdescription(parser, args):
    ...
    if gclient_utils.IsEnvCog():
        print(
            'description command is not supported. Please navigate to source '
            'control view in the activity bar to edit the CL description.',
            file=sys.stderr)
        return 1
...
def CMDweb(parser, args):
    ...
    if gclient_utils.IsEnvCog():
        print(
            'web command is not supported. Please use "Open Changes in '
            'Gerrit" command in the command palette in the Editor instead.',
            file=sys.stderr)
        return 1
