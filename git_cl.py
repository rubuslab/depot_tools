...
def IssueSwitch(options, args):
  ...
  `git cl issue -s issue_number`.  
  """
  issue_branch_map = IssueBranchMap()
  if not args:
    DieWithError(
        "`git cl issue --switch` requires an issue parameter argument.")
  try:
    issue_num = int(args[0])
  except ValueError:
    DieWithError("""`git cl issue --switch` requires an issue parameter argument
        which is a number""")
  branches = issue_branch_map[issue_num]
  if not branches:
    print('ERROR: No branch associated to issue: %d' % issue_num,
          file=sys.stderr)
  if len(branches) > 1:
    print('ERROR: Multiple branches associated to issue: %d' % issue_num,
          file=sys.stderr)
...
def CMDissue(parser, args):
  ...
  --reverse and --switch option allow to access branch(es) from issue.
  """
  ...
  parser.add_option('-s',
                    ...
                    'In case of multiple branches, behaves as `-r`. '
  ...
  if options.switch:
    return IssueSwitch(options, args)
