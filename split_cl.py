...
def UploadCl(
    ...
    """
    # Create a branch.
    if not CreateBranchForDirectories(
        refactor_branch, directories, refactor_branch_upstream
    ):
        print(
            "Skipping "
    ...
    print(
        "Uploading CL for " + FormatDirectoriesForPrinting(directories) + "..."
    )
    ...
    if comment:
        changelist().AddComment(
            FormatDescriptionOrComment(comment, directories), publish=True
        )
...
def GetFilesSplitByOwners(files, max_depth):
    ...
    for action, path in files:
        ...
        while (
            dir_with_owners not in files_split_by_owners
...
def SplitCl(
    ...
    try:
        ...
        for cl_index, (reviewers, cl_info) in enumerate(
            files_split_by_reviewers.items(), 1
        ):
