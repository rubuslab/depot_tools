...
def get_branches_info():
  ...
  with run_stream('for-each-ref', format_string, 'refs/heads', text=True) as eachRef:
    for line in eachRef:
      (branch, branch_hash, upstream_branch, tracking_status) = line.strip().split(':')
      commits = None
      behind_commits = None
      ...
      info_map[branch] = BranchesInfo(
          ...
          behind=behind_commits, upstream_gone=upstream_gone)
