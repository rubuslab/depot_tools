...
class GitWrapper(SCMWrapper):
  ...
  def apply_patch_ref(self, patch_repo, patch_ref, target_branch, options,
    ...
    what commits constitute the patch:
       ...
       target_branch
             |
        merge_base ---- patch_rev
             |
         base_rev

    The commits that constitute the patch are those in merge_base..patch_rev.

    Note that there might be several commits between base_rev and merge_base
    (in case we're applying the patch on top of an older revision than the
    branch was uploaded against), and between merge_base and patch_ref (in
    case of a chain of dependent CLs).
