...
class Flattener(object):
  ...
  def flatten(self):
    """Runs the flattener. Returns resulting DEPS string."""
  ...
  def _flatten_dep(self, dep):
    ...
    for hook_os, os_hooks in dep.os_deps_hooks.iteritems():
      self._hooks_os.setdefault(hook_os, []).extend([(dep, hook) for hook in os_hooks])
...
def CMDflatten(parser, args):
  ...
  flattened_deps = flattener.flatten()
