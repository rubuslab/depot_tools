...
def rebase_gerrit_ref(gerrit_repo, gerrit_ref, dest_branch, root, gerrit_reset):
  print '===Rebasing gerrit ref==='
  print 'Rebasing %r @ %r on top of %r, path is %r' % (
      gerrit_repo, gerrit_ref, dest_branch, path)
  try:
    git('rebase', dest_branch, cwd=path)
    if gerrit_reset:
...
def ensure_checkout(solutions, revisions, first_sln, target_os, target_os_only,
  ...
  if issue:
    ...
    # git repositories.
    rebase_gerrit_ref(gerrit_repo, gerrit_ref, patch_root, gerrit_reset,
                      gerrit_dest_branch)
...
def parse_args():
  ...
  parse.add_option('--gerrit_dest_branch',
                   help='Branch to rebase gerrit on top of')
  ...
  try:
    ...
    try:
      checkout_parameters = dict(
          ...
          gerrit_dest_branch=options.gerrit_dest_branch,
