...
class GitConfigStateBase(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def unset_config(self, key: str, *, scope: GitConfigScope, missing_ok: bool):
...
class GitConfigStateReal(GitConfigStateBase):
    ...
    def unset_config_multi(self, key: str, *, value_pattern: Optional[str],
        ...
        if value_pattern is not None:
            args.append(value_pattern)
        try:
            GIT.Capture(args, cwd=self.root, accepted_retcodes=accepted_retcodes)
        except subprocess2.CalledProcessError as cpe:
            if cpe.returncode == 5:
                raise GitConfigUnsetMissingValue(key, scope)
            raise
...
class GitConfigStateTest(GitConfigStateBase):
    ...
    def unset_config(self, key: str, *, scope: GitConfigScope, missing_ok: bool):
        cfg = self._get_scope(scope)
        cur = cfg.get(key)
        if cur is None and not missing_ok:
            raise GitConfigUnsetMissingValue(key, scope)
        if cur is None or len(cur) == 1:
            del cfg[key]
            return
