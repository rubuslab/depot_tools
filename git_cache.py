...
class Mirror(object):
  ...
  def bootstrap_repo(self, directory):
    ...
    # Download pack and idx file to a temporary directory.
    try:
      ...
      if code:
        return False
      filename_pack = os.path.join(tempdir, latest_checkout.split('/')[-1])
      filename_idx = filename_pack[:-5] + '.idx'
      try:
        shutil.move(filename_pack, directory)
        shutil.move(filename_idx, directory)
  ...
  def update_bootstrap(self, prune=False):
    ...
    _, tmp_idxfile = tempfile.mkstemp(suffix='.idx')
    os.remove(tmp_packfile)
    os.remove(tmp_idexfile)
    gsutil = Gsutil(path=self.gsutil_exe, boto_path=None)
    gs_folder = 'gs://%s/%s' % (self.bootstrap_bucket, self.basedir)
    dest_name_pack = '%s/%s.pack' % (gs_folder, gen_number)
    dest_name_idx = '%s/%s.pack' % (gs_folder, gen_number)
    gsutil.call('cp', tmp_packfile, dest_name_pack)
    gsutil.call('cp', tmp_idxfile, dest_name_idx)
    os.remove(tmp_packfile)
    os.remove(tmp_idxfile)
