...
def RunSteps(api):
  ...
  if api.tryserver.gerrit_change:
    ...
    assert api.tryserver.gerrit_change_patch_ref == 'refs/changes/27/91827/1'
    expected_dest_ref = api.properties.get('dest_ref', 'refs/heads/master')
    assert api.tryserver.gerrit_change_dest_ref == expected_dest_ref
...
def GenTests(api):
  ...
  yield (api.test('with_gerrit_patch_and_dest_ref') +
         api.buildbucket.try_build(
            'chromium',
            'linux',
            git_repo='https://chromium.googlesource.com/chromium/src',
            change_number=91827,
            patch_set=1) +
         api.properties(dest_ref='refs/heads/experiment') +
         api.tryserver.gerrit_change_dest_ref('refs/heads/experiment'))
