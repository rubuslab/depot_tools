...
class Changelist(object):
  ...
  def GetMostRecentDryRunPatchset(self):
    ...
    patchset = data['revisions'][data['current_revision']]['_number']
    #print("curr patchset", patchset)
    dry_run = set([m['_revision_number'] 
      for m in data.get('messages', []) if m['tag'].endswith('dry-run')])

    for revision_info in sorted(data['revisions'].values(), key=lambda c: c['_number'], reverse=True):
      if revision_info['_number'] in dry_run:
        patchset = revision_info['_number']
        break
      if revision_info.get('kind', []) not in ['NO_CHANGE', 'NO_CODE_CHANGE', 'TRIVIAL_REBASE']:
        break

    #print("returning patchset", patchset)
    self.SetPatchset(patchset)
    return patchset
