...
class Changelist(object):
  ...
  def _GetRefSpecOptions(self, options, change_desc, multi_change_upload=False):
    ...
    refspec_opts.extend(['hashtag=%s' % t for t in sorted(hashtags)])
...
def UploadAllSquashed(options, orig_args):
  ...
  refspec_opts = cl._GetRefSpecOptions(options)
  refspec_suffix = ''
  if refspec_opts:
    refspec_suffix = '%' + ','.join(refspec_opts)
  ...
  push_stdout = last_cl._RunGitPushWithTraces(refspec, refspec_opts,
                                              git_push_metadata)
