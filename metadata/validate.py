...
from metadata.validation_result import ValidationResult


def find_readmes(root: str) -> List[str]:
  """Finds all files with the name `README.chromium` within the given
  root directory, including subdirectories.

  Args:
    root: the root directory to search with.

  Returns:
    a list of all the README.chromium filepaths within the root directory.
  """
  readmes = []
...
def validate_file(filepath: str,
  ...
  """
  readme_dir = os.path.dirname(filepath)
  results = []
  for dependency_metadata in parse_file(filepath):
...
def main() -> None:
  ...
  field_issues = defaultdict(lambda: defaultdict(int))
  for filepath in readmes:
    file_results = validate_file(filepath, chromium_src_dir=src_dir)
    invalid = False
    if file_results:
      ...
      for result in file_results:
        ...
        if field_tag:
          field_issues[field_tag][str(result)] += 1
