...
class GclientApi(recipe_api.RecipeApi):
  ...
  def get_gerrit_patch_root(self, gclient_config=None):
    ...
    if not repo_url:
      return None
    root =  self.get_repo_path(repo_url, gclient_config=gclient_config)
    if not root:  # pragma: no cover
      raise self.m.step.InfraFailure(
          '%s does not match any of gclient_config.solutions '
          'and not present in gclient_config.repo_path_map' % repo_url)
    return root
