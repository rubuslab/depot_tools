...
class GClientSmokeGIT(gclient_smoketest_base.GClientSmokeBase):
  ...
  def testNoSync_SkipSyncNoDEPSChange(self):
    ...
    self.env[gclient.PREVIOUS_SYNC_COMMITS] = json.dumps(
        {'src': revision_1})
    self.env[gclient.PREVIOUS_CUSTOM_VARS] = json.dumps(
        {'src': {'mac': True}})
    ...
    self.parseGclient(
        ['sync',
         ...
         '--revision', 'src@%s' % revision_2,
         '--patch-ref', '%srepo_1@refs/heads/main:%s' % (
             self.git_base, patch_ref)
         ], [])
    ...
    self.assertEqual(out, output_json['solutions']['src/'])
