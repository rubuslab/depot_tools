...
"""Download files from Google Storage, given the bucket and path."""
...
def main(args):
    ...
    parser.add_option('-p', '--path', help='Path inside bucket to fetch.')
    parser.add_option('-o',
                      '--output',
    ...
    path_url = 'gs://%s/%s' % (options.bucket, options.path)
    ...
    gsutil.check_call('cp', path_url, options.output)
