...
def CheckChangedLUCIConfigs(input_api, output_api):
  ...
  for f in input_api.AffectedFiles(include_deletes=False):
    for d in dir_to_config_set:
      if d == '.' or f.LocalPath().startswith(d):
        files = dir_to_files.setdefault(d, [])
        files.append(f)
  ...
  for d, files in dir_to_files.items():
    ...
    with input_api.CreateTemporaryFile() as f:
      cmd = [
          'lucicfg' if not input_api.is_windows else 'lucicfg.bat', 'validate',
          d, '-config-set', config_set, '-log-level',
          'debug' if input_api.verbose else 'warning', '-json-output', f.name
      ]
      for file in files:
        cmd.append('-file-filters')
        glob = file.LocalPath() if d == '.' else input_api.os_path.relpath(
            file.LocalPath(), start=d)
        cmd.append(glob.replace(_os.sep, '/'))
      ...
      try:
        ...
        for validation_result in result.get('result', {}).get('validation', []):
