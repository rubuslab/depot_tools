...
class FilePatchBase(object):
  ...
  def __init__(self, filename):
    ...
    self.patchlevel = 0
    self.decrement_patchlevel = False
  ...
  def filename_after_patchlevel(self):
    ...
    if self.decrement_patchlevel:
      patchlevel -= 1
    return self._apply_patchlevel(self.filename, patchlevel)
  ...
  def _verify_git_header(self):
    ...
    while lines:
      ...
      if match.group(1).startswith('a/') and match.group(2).startswith('b/'):
        # When dealing with regular files, we should use patchlevel - 1, since
        # "a/" is not part of the file path.
        self.decrement_patchlevel = True
        self.patchlevel = 1
