...
class GerritClient(OwnersClient):
  ...
  def _FetchOwners(self, path, cache, highest_score_only=False):
    ...
    if path not in cache:
      ...
      data = gerrit_util.GetOwnersForFile(
          ...
          resolve_all_users=False, highest_score_only=highest_score_only, seed=self._seed)
      cache[path] = [
        d['account']['email']
  ...
  def ListBestOwners(self, path):
    return self._FetchOwners(path, self._best_owners_cache, highest_score_only=True)
