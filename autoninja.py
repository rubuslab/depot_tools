...
def main(args):
    ...
    if os.path.exists(os.path.join(output_dir, "args.gn")):
        with open(os.path.join(output_dir, "args.gn")) as file_handle:
            for line in file_handle:
                ...
                if re.search(
                    r"(^|\s)(use_goma)\s*=\s*true($|\s)", line_without_comment
                ):
                    use_goma = True
                    continue
                if re.search(
                    ...
                    continue
                if re.search(
                    r"(^|\s)(use_rbe)\s*=\s*true($|\s)", line_without_comment
                ):
                    use_rbe = True
                    continue
                if re.search(
                    r"(^|\s)(use_siso)\s*=\s*true($|\s)", line_without_comment
                ):
                    use_siso = True
                    continue
        ...
        if use_siso:
            ...
            if os.path.exists(ninja_marker) and not os.path.exists(siso_marker):
                return (
                    "echo Run gn clean before switching from ninja to siso in %s"
                    % output_dir
                )
    ...
    if use_goma:
        ...
        goma_dir = os.environ.get(
            "GOMA_DIR", os.path.join(SCRIPT_DIR, ".cipd_bin")
        )
    ...
    offline_env = (
        ["RBE_remote_disabled=1", "GOMA_DISABLED=1"]
    ...
    if sys.platform == "darwin":
        ...
        if fileno_limit <= wanted_limit:
            try:
                resource.setrlimit(
                    resource.RLIMIT_NOFILE, (wanted_limit, hard_limit)
                )
