...
def _RunRustFmt(opts, rust_diff_files, top_dir, upstream_commit):
  ...
  try:
    rustfmt_tool = rust_support.FindRustfmtToolInChromiumTree()
  except rust_support.NotFoundError as e:
    DieWithError(e)

  # TODO(lukasza): Support formatting only the changed lines if
  # `opts.full or settings.GetFormatFullByDefault()` is False.
  # See also: https://github.com/emilio/rustfmt-format-diff
  cmd = [rustfmt_tool]
  if opts.dry_run:
    cmd.append('--check')
  cmd += rust_diff_files
...
def CMDformat(parser, args):
  ...
  parser.add_option(
      ...
      dest='use_rust_fmt',
      action='store_true',
      default=rust_support.IsRustSupported(),
      help='Enables formatting of various file types using rustfmt.')
  parser.add_option(
      '--no-rust-fmt',
      dest='use_rust_fmt',
      action='store_false',
