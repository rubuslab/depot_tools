...
def CheckLongLines(input_api, output_api, maxlen, source_file_filter=None):
  ...
  def is_global_pylint_directive(line, pos):
    ...
    # Any character before |pos| that is not whitespace of # indidcates
    # this is a local directive.
    return any([c for c in line[:pos] if c in " \t#"])
  ...
  def check_python_long_lines(affected_files, error_formatter):
    ...
    for f in affected_files:
      ...
      for idx, line in enumerate(f.NewContents()):
        ...
        if do_check:
          #raise Exception('%d:%s' % (line_num, line))
  ...
  non_py_file_ext_list = [x for x in file_ext_list if x[1] != 'py']
  ...
  py_file_list = [x[0] for x in file_ext_list if x[1] == 'py']
  if py_file_list:
