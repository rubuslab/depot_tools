  ...
  def CMDUploadChange(self, options, args, change):
    ...
    if options.squash:
      ...
      if self.GetIssue():
        ...
        if not title:
          if options.message:
            # For compatibility with Rietveld, if -m|--message is given on command
            # line, title should be the first line of that message, which
            # shouldn't be confused with CL description.
    ...
    # TODO(tandrii): options.message should be post a comment if --send-email
    # is set on non-initial upload as Rietveld used to do it.
