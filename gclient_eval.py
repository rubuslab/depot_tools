...
def _MergeDepsOs(deps_dict, os_deps_dict, os_name):
  ...
  for dep_name, dep_info in os_deps_dict.items():
    ...
    UpdateCondition(dep_info, 'and', 'checkout_' + os_name)
...
def UpdateCondition(info_dict, op, new_condition):
  ...
  if condition and new_condition:
    info_dict['condition'] = '(%s) %s (%s)' % (condition, op, new_condition)
  # If one of them is absent, and |op| == 'or', we treat the absent one as
  # always True, and so no condition is necessary.
  elif op == 'or' and condition:
    del info_dict['condition']
  # If info_dict's condition is absent, but new_condition is present, and
  # |op| == 'and', then use the new condition.
  elif op == 'and' and new_condition:
    info_dict['condition'] = new_condition
