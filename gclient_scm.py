...
class GitWrapper(SCMWrapper):
  ...
  def apply_patch_ref(self, patch_repo, patch_ref, target_ref, options,
    ...
    if not target_ref:
      target_ref = self._GetTargetBranchForCommit(base_rev) || base_rev
    elif not target_ref.startswith(('refs/heads/', 'refs/branch-heads/')):
      raise gclient_utils.Error(
          'target_ref must be a refs/heads/ or a refs/branch-heads/. '
          'Got %s instead.' % target_ref)
    elif target_ref == 'refs/heads/master':
      # Most of the time, we'll be applying patches against master, so handle
      # this separately.
      target_ref = 'refs/remotes/%s/master' % self.origin
    elif not scm.GIT.IsValidRevision(target_ref):
      # The second most common case is applying patchos for the top-level
      # solution, where the top level solution is synced to the target ref of
      # the change.
      # In this case, bot_update.py will fetch the target ref and we won't need
      # to execute this code.
      # We check next for a remote version of the target ref, and fetch it if
      # it's not available.
      target_ref = ''.join(scm.GIT.RefToRemoteRef(target_ref, self.remote))
      if not scm.GIT.IsValidRevision(target_ref):
        url, _ = gclient_utils.SplitUrlRevision(self.url)
        mirror = self._GetMirror(url, options, target_ref)
        if mirror:
          self._UpdateMirrorIfNotContains(mirror, options, 'branch', target_ref)
