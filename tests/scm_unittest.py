...
class GitConfigStateTestTest(unittest.TestCase):
    ...
    def test_construction_system(self):
        ...
        self.assertDictEqual(gs, {'section.key': ['global']})
        self.assertDictEqual(m.load_config(), {'section.key': ['system', 'global']})

        gs['section.key'] = ['override']
        self.assertDictEqual(m.load_config(), {'section.key': ['system', 'override']})
    ...
    def test_set_config_system(self):
        ...
        with self.assertRaises(scm.GitConfigUneditableScope):
            m.set_config('section.key', 'new_global', append=False, scope='system')

    def test_set_config_unknown(self):
        m, _ = self._make()

        with self.assertRaises(scm.GitConfigUnknownScope):
            m.set_config('section.key', 'new_global', append=False, scope='meepmorp')
