...
def get_luci_context_access_token():
  ...
  # if account_id or secret not specified, LUCI_CONTEXT should not be picked up
  ...
  if not expiry:
    authErr('local_auth: No expiry datetime returned for token')
  try:
  ...
  if _needs_refresh(access_token):
