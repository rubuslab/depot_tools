...
class GerritChangesTest(fake_repos.FakeReposTestBase):
  ...
  def testCheckoutOriginFeaturePatchBranch(self):
    ...
    # 'origin/feature'.
    # This is meant to test that the target_branch argument works, not that
    # applying a patch for branch X in branch Y would work in real life. It just
    # so happens that they share a merge base in this test.
    scm.apply_patch_ref(self.url, 'refs/changes/34/1234/1', 'origin/feature',
                        self.options, file_list)

    self.assertCommits([1, 2, 3, 5, 7, 8, 9])
    self.assertEqual(self.githash('repo_1', 9), self.gitrevparse(self.root_dir))
