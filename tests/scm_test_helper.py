...
class GitConfigStateTest(scm.GitConfigState):
    ...
    def _unset_config(self, key: str, scope: GitConfigScope):
        ...
        if cur is None or len(cur) == 1:
            ...
            return
        raise ValueError(
                f'GitConfigStateTest: Cannot unset key {key} '
                f'- current value {cur!r} is multiple.')
...
def mock_GIT(test: unittest.TestCase, config: Optional[Dict[str, List[str]]] = None, branchref: Optional[str] = None):
    ...
    mock.patch('scm.GIT._new_config_state', side_effect=lambda root:
               GitConfigStateTest(root, default=config)).start()
    mock.patch('scm.GIT.GetBranchRef', side_effect=lambda _root:
               _branchref[0]).start()
    def _newBranch(branchref):
        _branchref[0] = branchref
    mock.patch('git_new_branch.create_new_branch', side_effect=_newBranch).start()
