...
class Dependency(gclient_utils.WorkItem, DependencySettings):
  ...
  def _OverrideUrl(self):
    ...
    if parsed_url != self.url:
      logging.info('Dependency(%s).OverrideUrl(%s) -> %s', self._name,
                   self.url, parsed_url)
      ...
      if (not parsed_url[0] and
        ...
        logging.info('Dependency(%s).OverrideUrl(%s) -> %s', self.name,
                     self.url, parsed_url)
        self.set_url(parsed_url)

    elif self.url is None:
      logging.info('Dependency(%s).OverrideUrl(None) -> None', self._name)
