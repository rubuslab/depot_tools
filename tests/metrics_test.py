...
class MetricsCollectorTest(unittest.TestCase):
  ...
  def assert_collects_metrics(self, update_metrics=None):
    ...
    self.assertEqual(self.FileWrite.call_args[0][0], '/tmp/metrics.json')
    self.assertEqual(
        json.loads(self.FileWrite.call_args[0][1]),
        expected_metrics)
    # Assert we invoked the script to upload them.
    self.Popen.assert_called_with(
        [sys.executable, metrics.UPLOAD_SCRIPT, '/tmp/metrics.json'],
        stdout=metrics.subprocess.PIPE, stderr=metrics.subprocess.PIPE)
