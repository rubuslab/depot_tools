...
class Authenticator(object):
  ...
  def get_access_token(self, force_refresh=False, allow_user_interaction=False,
    ...
    Raises:
      ...
      LuciContextAuthError if use_luci_context is True but fails to read in
          LUCI_CONTEXT due to either it not existing or incorrectly formatted.
    """
    def get_luci_ctx_ack_tkn():
      acc_tkn = get_luci_context_access_token()
      if not acc_tkn:
        raise LuciContextAuthError('LUCI_CONTEXT not set')
      return acc_tkn
    ...
    with self._lock:
      if force_refresh:
        ...
        except (AuthenticationError, LoginRequiredError):
          logging.exception('Failed to create access token')
          if use_luci_context:
            self._access_token = get_luci_ctx_ack_tkn()
            return self._access_token
          else:
            raise
