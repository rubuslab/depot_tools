...
class Dependency(gclient_utils.WorkItem, DependencySettings):
  ...
  def _OverrideUrl(self):
    ...
    if parsed_url != self.url:
      logging.info('Dependency(%s).LateOverride(%s) -> %s', self._name,
                   self.url, parsed_url)
      ...
      if (not parsed_url[0] and
        ...
        logging.info('Dependency(%s).LateOverride(%s) -> %s', self.name,
                     self.url, parsed_url)
        self.set_url(parsed_url)

    elif self.url is None:
      logging.info('Dependency(%s).LateOverride(None) -> None', self._name)
...
'''  % {'checkout_path': os.path.join(self.root_dir, dep.name),
  ...
  def PrintRevInfo(self):
    ...
    def GetURLAndRev(dep):
      ...
      if dep.url is None:
        return None
      url, _ = gclient_utils.SplitUrlRevision(dep.url)
      scm = dep.CreateSCM(
          dep.url, self.root_dir, dep.name, self.outbuf)
...
class Flattener(object):
  ...
  def _pin_dep(self, dep):
    ...
    url, revision = gclient_utils.SplitUrlRevision(dep.url)
    if revision and gclient_utils.IsFullGitSha(revision):
      return

    scm = dep.CreateSCM(
        dep.url, self._client.root_dir, dep.name, dep.outbuf)
    revinfo = scm.revinfo(self._client._options, [], None)

    dep.set_url('%s@%s' % (url, revinfo))
