...
class GclientApi(recipe_api.RecipeApi):
  ...
  def roll_deps(self, deps_content, dep_updates, strip_prefix_for_gitlink=None):
    """Updates DEPS file to desired revisions, and returns all requried file
    changes.

    Args:
      deps_content - Content of DEPS file
    ...
    deps_path = tempdir.join('DEPS')
    self.m.file.write_text('Write to %s' % deps_path, deps_path, deps_content)
