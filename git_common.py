...
def warn_submodule():
    ...
    if sys.platform.startswith("darwin") and is_fsmonitor_enabled():
        ...
        print(
            "We will remove this warning once https://crbug.com/1475405 is "
            "fixed."
        )
...
def is_dirty_git_tree(cmd):
    ...
    if dirty:
        ...
        if len(dirty) > 4096:  # pragma: no cover
            w(
                '... (run "git diff-index --name-status HEAD" to see full output).'
            )
        return True
    return False
...
def squash_current_branch(header=None, merge_base=None):
    ...
    if log_msg:
        log_msg += "\n"
    log_msg += run(
        "log", "--reverse", "--format=%H%n%B", "%s..HEAD" % merge_base
    )
    ...
    if not get_dirty_files():
        ...
        return False
    run(
        "commit", "--no-verify", "-a", "-F", "-", indata=log_msg.encode("utf-8")
    )
    return True
