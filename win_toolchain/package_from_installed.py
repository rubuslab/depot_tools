...
def BuildFileList(override_dir):
  ...
  if VS_VERSION == '2017':
    paths += [
      'VC/Tools/MSVC/14.10.25017/atlmfc',
      'VC/Tools/MSVC/14.10.25017/crt',
    ]
  else:
    paths += [
      'VC/atlmfc',
  ...
  if override_dir:
    ...
    if VS_VERSION == '2017':
      paths += [
          ...
          'VC/Tools/MSVC/14.10.25017/include',
          'VC/Tools/MSVC/14.10.25017/lib',
      ]
    else:
      paths += [
          'VC/bin',
  ...
  for path in paths:
    ...
    if not os.path.exists(combined) or not os.path.isdir(combined):
      raise Exception("Missing or not directory %s" % combined)
...
def GenerateSetEnvCmd(target_dir):
  ...
  if VS_VERSION == '2017':
    include_dirs.extend([
      ['..', '..', 'VC', 'Tools', 'MSVC', '14.10.25017', 'include'],
      ['..', '..', 'VC', 'Tools', 'MSVC', '14.10.25017', 'atlmfc', 'include'],
    ])
  else:
    include_dirs.extend([
      ['..', '..', 'VC', 'include'],
  ...
  if VS_VERSION == '2017':
    env_x86 = collections.OrderedDict([
      ('PATH', [
        ['..', '..', 'win_sdk', 'bin', 'x64'],
        ['..', '..', 'VC', 'Tools', 'MSVC', '14.10.25017', 'bin', 'HostX64', 'x86'],
        ['..', '..', 'VC', 'Tools', 'MSVC', '14.10.25017', 'bin', 'HostX64', 'x64'],  # Needed for mspdb1x0.dll.
      ]),
      ('LIB', [
        ['..', '..', 'VC', 'Tools', 'MSVC', '14.10.25017', 'lib', 'x86'],
        ['..', '..', 'win_sdk', 'Lib', WIN_VERSION, 'um', 'x86'],
        ['..', '..', 'win_sdk', 'Lib', WIN_VERSION, 'ucrt', 'x86'],
        ['..', '..', 'VC', 'Tools', 'MSVC', '14.10.25017', 'atlmfc', 'lib', 'x86'],
      ]),
    ])
  else:
    env_x86 = collections.OrderedDict([
  ...
  if VS_VERSION == '2017':
    env_x64 = collections.OrderedDict([
      ('PATH', [
        ['..', '..', 'win_sdk', 'bin', 'x64'],
        ['..', '..', 'VC', 'Tools', 'MSVC', '14.10.25017', 'bin', 'HostX64', 'x64'],
      ]),
      ('LIB', [
        ['..', '..', 'VC',  'Tools', 'MSVC', '14.10.25017', 'lib', 'x64'],
        ['..', '..', 'win_sdk', 'Lib', WIN_VERSION, 'um', 'x64'],
        ['..', '..', 'win_sdk', 'Lib', WIN_VERSION, 'ucrt', 'x64'],
        ['..', '..', 'VC',  'Tools', 'MSVC', '14.10.25017', 'atlmfc', 'lib', 'x64'],
      ]),
    ])
  else:
    env_x64 = collections.OrderedDict([
...
def main():
  usage = 'usage: %prog [options] 2013|2015|2017'
  ...
  if len(args) != 1 or args[0] not in ('2013', '2015', '2017'):
    print 'Must specify 2013, 2015, or 2017'
    parser.print_help();
    return 1
