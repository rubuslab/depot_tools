...
class InputApiUnittest(PresubmitTestsBase):
  ...
  def testDefaultOverrides(self):
    input_api = presubmit.InputApi(
        self.fake_change, './PRESUBMIT.py', False, None, False)
    self.assertEqual(len(input_api.DEFAULT_CHECK_LIST), 24)
    self.assertEqual(len(input_api.DEFAULT_SKIP_LIST), 12)
    self.assertEqual(input_api.DEFAULT_CHECK_LIST, input_api.DEFAULT_WHITE_LIST)
    self.assertEqual(input_api.DEFAULT_CHECK_LIST, input_api.DEFAULT_ALLOW_LIST)
    self.assertEqual(input_api.DEFAULT_SKIP_LIST, input_api.DEFAULT_BLACK_LIST)
    self.assertEqual(input_api.DEFAULT_SKIP_LIST, input_api.DEFAULT_BLOCK_LIST)

    input_api.DEFAULT_CHECK_LIST = (r'.+\.c$',)
    input_api.DEFAULT_SKIP_LIST = (r'.+\.patch$', r'.+\.diff')
    self.assertEqual(len(input_api.DEFAULT_CHECK_LIST), 1)
    self.assertEqual(len(input_api.DEFAULT_SKIP_LIST), 2)
    self.assertEqual(input_api.DEFAULT_CHECK_LIST, input_api.DEFAULT_WHITE_LIST)
    self.assertEqual(input_api.DEFAULT_CHECK_LIST, input_api.DEFAULT_ALLOW_LIST)
    self.assertEqual(input_api.DEFAULT_SKIP_LIST, input_api.DEFAULT_BLACK_LIST)
    self.assertEqual(input_api.DEFAULT_SKIP_LIST, input_api.DEFAULT_BLOCK_LIST)
    ...
    self.assertEqual(len(input_api.DEFAULT_CHECK_LIST), 0)
    self.assertEqual(len(input_api.DEFAULT_SKIP_LIST), 0)
    self.assertEqual(input_api.DEFAULT_CHECK_LIST, input_api.DEFAULT_WHITE_LIST)
    self.assertEqual(input_api.DEFAULT_CHECK_LIST, input_api.DEFAULT_ALLOW_LIST)
    self.assertEqual(input_api.DEFAULT_SKIP_LIST, input_api.DEFAULT_BLACK_LIST)
    self.assertEqual(input_api.DEFAULT_SKIP_LIST, input_api.DEFAULT_BLOCK_LIST)
  ...
  def testLambdaFilter(self):
    check_list = presubmit.InputApi.DEFAULT_SKIP_LIST + (r".*?a.*?",)
    skip_list = [r".*?b.*?"]
    ...
    got_files = input_api.AffectedSourceFiles(
        lambda x: input_api.FilterSourceFile(x, check_list, skip_list))
    self.assertEqual(len(got_files), 2)
    self.assertEqual(got_files[0].LocalPath(), 'eeaee')
