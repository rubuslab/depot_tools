...
import threading
from typing import Iterable, Literal, Dict, List, Optional, Tuple, Mapping
...
class GitConfigStateBase(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def load_config(self) -> Mapping[str, List[str]]:
    ...
    @abc.abstractmethod
    def set_config(self, key: str, value: str, append: bool, scope: GitConfigScope):
    ...
    def unset_config_multi(self, key: str, value_pattern: Optional[str],
        ...
        """When invoked, remove a all values from `key` in this state's underlying data.
...
class CachedGitConfigState:
    ...
    def __init__(self, impl: GitConfigStateBase):
        ...
        self._config: Optional[Mapping[str, List[str]]] = None

    def _maybe_load_config(self) -> Mapping[str, List[str]]:
        if self._config is None:
            self._config = self._impl.load_config()
        if self._config is None:
            raise AssertionError(f'{self.__class__.__name__} failed to populate'
                                 ' CachedGitConfigState._config')
        return self._config
    ...
    def SetConfig(self,
        ...
        if value is None:
            ...
            if modify_all:
                self._impl.unset_config_multi(key, value_pattern, scope)
            else:
                self._impl.unset_config(key, scope)
        else:
            if modify_all:
                self._impl.set_config_multi(key, value, append, value_pattern, scope)
            else:
                self._impl.set_config(key, value, append, scope)
...
class GitConfigStateTest(GitConfigStateBase):
    def __init__(self, initial_state: Optional[Dict[GitConfigScope, Dict[str, List[str]]]]):
        super()
