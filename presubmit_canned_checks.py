...
def GetUnitTestsInDirectory(
    ...
    python2_only=False, python3_only=False):
  ...
  return GetUnitTests(
      input_api, output_api, unit_tests, env, python2_only, python3_only)


def GetUnitTests(
    input_api, output_api, unit_tests, env=None, python2_only=False,
    python3_only=False):
  ...
  for unit_test in unit_tests:
    ...
    if not unit_test.endswith('.py'):
      ...
      if has_py3_shebang(unit_test) and not python2_only:
        results.append(input_api.Command(
            name=unit_test,
            cmd=cmd,
            kwargs=kwargs,
