...
class ChangelistMock(object):
  ...
  def GetIssue(self):
    return 1
  def GetRemoteDescription(self):
    return ChangelistMock.desc
  def UpdateRemoteDescription(self, desc, force=False):
...
class TestGitClBasic(unittest.TestCase):
  def test_get_description(self):
    ...
    self.assertEqual(cl.GetRemoteDescription(), 'x')
...
class TestGitCl(TestCase):
  ...
  def test_StatusFieldOverrideIssue(self):
    ...
    self.mock(git_cl.Changelist, 'GetRemoteDescription', assertIssue)
    self.assertEqual(
  ...
  def test_description_appends_bug_line(self):
    ...
    def UpdateRemoteDescription(_, desc, force=False):
      self.assertEqual(desc, 'Some.\n\nChange-Id: xxx\nBug: 123')

    self.mock(git_cl.sys, 'stdout', StringIO())
    self.mock(git_cl.Changelist, 'GetRemoteDescription',
              lambda *args: current_desc)
    self.mock(git_cl.Changelist, 'UpdateRemoteDescription',
  ...
  def test_gerrit_description_caching(self):
    ...
    cl._cached_remote_url = (
        True, 'https://chromium.googlesource.com/a/my/repo.git/')
    self.assertEqual(cl.GetRemoteDescription(), 'desc1')
    self.assertEqual(cl.GetRemoteDescription(), 'desc1')  # cache hit.
