...
class CachedGitConfigState(object):
    ...
    def GetConfig(self,
        ...
        if scoped_config == None:

            return default

        scoped_config = self._maybe_load_config().get(scope, None)
    ...
    def YieldConfigRegexp(self,
        ...
        for key, values in sorted(self._maybe_load_config()["default"].items()):
            if pred(key):
                for value in values:
                    yield key, value
    ...
    def load_config(self) -> GitFlatConfigData:
        ...
        while i < len(entries):
            if entries[i] in ['local', 'global', 'system']:
                # This entry starts with a scope
                scope = entries[i]
                i += 1
                if i < len(entries):
                    line = entries[i]
                    ...
                    if len(parts) == 2:
                        ...
                        key, value = parts[0], ''
                    cfg[scope][key.strip()].append(value.strip())
                    cfg["default"][key.strip()].append(value.strip())
            else:
                line = entries[i]
                parts = line.split('\n', 1)
                if len(parts) == 2:
    ...
    def _dump_config_state(cls) -> Dict[str, GitFlatConfigData]:
        ...
        with cls._CONFIG_CACHE_LOCK:
            ...
            for key, val in cls._CONFIG_CACHE.items():
                if val is not None:
                    state[str(key)] = val._maybe_load_config()["default"]
        return state
