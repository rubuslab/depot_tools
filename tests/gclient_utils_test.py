...
class AnnotatedTestCase(unittest.TestCase):
  def setUp(self):
    ...
    self.outz = gclient_utils.MakeFileAnnotated(io.BytesIO(), include_zero=True)

  def testString(self):
    self.outz.write('test string\n')
    self.assertEqual(self.outz.getvalue(), b'0>test string\n')

  def testBytes(self):
    self.out.write(b'test string\n')
    self.assertEqual(self.out.getvalue(), b'test string\n')
  ...
  def testUnicodeZero(self):
    self.outz.write('✔\n')
    self.assertEqual(self.outz.getvalue(), b'0>\xe2\x9c\x94\n')

  def testMultiline(self):
    self.out.write(b'first line\nsecond line')
    self.assertEqual(self.out.getvalue(), b'first line\nsecond line')

  def testFlush(self):
    self.outz.write(b'first line\nsecond line')
    self.assertEqual(self.outz.getvalue(), b'0>first line\n')
    self.outz.flush()
    self.assertEqual(self.outz.getvalue(), b'0>first line\n0>second line\n')
