...
KNOWN_HTTP_PATH_RES = (
  ...
  re.compile(r'/changes/\d+$'),
	re.compile(r'/changes/.*/abandon'),
	re.compile(r'/changes/.*/comments'),
	re.compile(r'/changes/.*/detail'),
	re.compile(r'/changes/.*/edit'),
	re.compile(r'/changes/.*/message'),
	re.compile(r'/changes/.*/restore'),
	re.compile(r'/changes/.*/reviewers/.*'),
	re.compile(r'/changes/.*/revisions/.*/commit'),
	re.compile(r'/changes/.*/revisions/.*/review'),
	re.compile(r'/changes/.*/submit'),
	re.compile(r'/projects/.*/branches/.*'),
)
...
def extract_http_metrics(request_uri, method, status, response_time):
  ...
  Extracts the host, path, and arguments fram the connection, and returns them
  along with the method, status and response time.

  The host, method, paths and arguments must be in the KNOWN_HTTP_* constants
  defined above.
  ...
  paths = [
    path_re.pattern for path_re in KNOWN_HTTP_PATH_RES
    if path_re.match(parsed_url.path)
  ]
  if paths:
    http_metrics['paths'] = paths
