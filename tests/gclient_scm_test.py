...
class GerritChangesTest(fake_repos.FakeReposTestBase):
  ...
  @mock.patch('gerrit_util.QueryChanges', return_value=[
      {'_number': 1234, 'revisions': [1]}, {'_number': 1235, 'revisions': [1]}])
  def testDownloadsTopics(self, query_changes_mock, get_change_mock):
    scm = gclient_scm.GitWrapper(self.url, self.root_dir, '.')
    file_list = []
    ...
    get_change_mock.assert_called_once_with(
        'repo_1-review.googlesource.com', '1234')
    query_changes_mock.assert_called_once_with(
        'repo_1-review.googlesource.com',
        [('topic', 'test_topic'), ('status', 'open')],
