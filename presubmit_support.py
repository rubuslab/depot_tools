...
class InputApi(object):
  ...
  def CreateTemporaryFile(self, bufsize=-1, suffix='', prefix='tmp', dir=None):
    """Returns a named temporary file that must be removed with a call to
    RemoveTemporaryFiles().

    |bufsize|, |suffix|, |prefix| and |dir| are passed to
    ...
    code.
    """
    temp_file = self.tempfile.NamedTemporaryFile(
      delete=False, bufsize=bufsize, suffix=suffix, prefix=prefix, dir=dir)
    self.named_temporary_files.append(temp_file.name)
    return temp_file

  def RemoveTemporaryFiles(self):
    """Removes all temporary files created with CreateTemporaryFile()."""
    while len(self.named_temporary_files):
      os.remove(self.named_temporary_files.pop())
