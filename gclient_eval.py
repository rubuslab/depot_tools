...
def Exec(content, filename='<unknown>', vars_override=None, builtin_vars=None):
  ...
  # In Python 2.7 the thing being tokenized needs to end with a newline.
  # Otherwise tokenize-internal assertions will happen in later usage.
  # In Python >3 this appending a newline is done automatically so it has no
  # effect.
  content += '\n'
  tokens = {
...
def AddVar(gclient_dict, var_name, value):
  ...
  # Update the tokens.
  var_tokens = list(tokenize.generate_tokens(io.BytesIO(var_content).readline))
  var_tokens = {
      token[2]: list(token)
