...
rem This batch file will try to sync the root directory.

setlocal

rem Windows freaks out if a file is overwritten while it's being executed.  Copy
rem this script off to a temporary location and reinvoke from there before
rem running any git commands.
...
rem Download git for the first time if it's not present.
...
:GIT_UPDATE
rem Now clear errorlevel so it can be set by other programs later.
set errorlevel=
...
rem depot_tools.zip archives generated before 2021-03-12 have instruction to
rem fetch  only from old default git branch. Such branch won't be available
rem evenutally, so fetch config needs to be updated.
...
rem Sync CIPD and CIPD client tools.
call "%~dp0\cipd_bin_setup.bat"

rem Update git and python
call "%DEPOT_TOOLS_DIR%bootstrap\win_tools.bat"
