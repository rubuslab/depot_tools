...
def CMDflatten(parser, args):
  ...
  if not options.output_deps:
    parser.error('--output-deps is required for flatten')

  options.nohooks = True
  client = GClient.LoadCurrentConfig(options)

  code = client.RunOnDeps('flatten', args)
  if code != 0:
    return code
...
def _FlattenSolution(solution, deps, hooks, pre_deps_hooks, unpinned_deps):
  ...
  Out-parameters:
    deps (dict of str -> Dependency): will be filled with flattened deps
    ...
    pre_deps_hooks (list of (Dependency, hook)): will be filled with flattened
        pre_deps_hooks
    unpinned_deps (dict of str -> str): will be filled with unpinned deps
  """
  ...
  hooks.extend([(solution, hook) for hook in solution.deps_hooks])
  pre_deps_hooks.extend([(solution, hook) for hook in solution.pre_deps_hooks])
...
def _AddDep(dep, deps, unpinned_deps):
  ...
  if isinstance(dep.url, basestring):
    ...
    # TODO(phajdan.jr): handle all kinds of url implementations,
    # including FromImpl.
    logging.warn('unimplemented handling of %r for %r', dep.url, dep)
...
def _DepsToLines(deps):
  ...
  for name in sorted(deps):
    s.extend([
        '  # %s' % deps[name].hierarchy(include_url=False),
        '  "%s": "%s",' % (name, deps[name].url),
        '',
    ])
  s.extend(['}', ''])
  return s
