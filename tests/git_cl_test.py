...
class TestGitCl(unittest.TestCase):
  ...
  def test_upload_all_squashed_dependencies(self, mockUploadAllSquashed,
    ...
    unrelated-branch
    '''
    current_cl = git_cl.Changelist(branchref='refs/heads/A')
    options = optparse.Values()
    orig_args = []
    git_cl.UploadAllSquashedDependencies(current_cl, options, orig_args)

    self.assertEqual(
        mockRunGit.mock_calls[1:],
        [
            mock.call(['checkout', '-q', 'C']),
            mock.call(['checkout', '-q', 'C2']),
            mock.call(['checkout', '-q', 'B2']),
            # final checkout back to root
            mock.call(['checkout', '-q', 'A']),
        ])
    self.assertEqual(len(mockUploadAllSquashed.mock_calls), 3)
