  ...
  def ensure_checkout(self,
    ...
    try:
      ...
      if step_result and step_result.json.output:
        ...
        # failed. This is to differentiate between an infra failure independent
        # of a patchset and a patch failure caused by the patchset author.
        if result.get('patch_failure'):
          ...
          if result.get('patch_apply_return_code') == 3:
            ...
            self.m.python.failing_step(
                'Patch failure',
                'See patch error log attached to bot_update. Try rebasing?')
