...
class PackageDefinition(object):
  ...
  def __init__(self, package_name, package_root, install_mode=None):
    ...
    if install_mode not in (None, 'copy', 'symlink'):  # pragma: no cover
  ...
  def add_dir(self, dir_path, exclusions=None):
    ...
    for e in exclusions:
      re.compile(e)  # make sure its a valid regex
  ...
  def add_version_file(self, ver_file_rel):
    ...
    if self.version_file is not None:  # pragma: no cover
...
class CIPDApi(recipe_api.RecipeApi):
  ...
  def initialize(self):
    self._executable = 'cipd' + ('.bat' if self.m.platform.is_win else '')
