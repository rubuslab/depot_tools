...
def UploadAllSquashed(options, orig_args):
  ...
  if cherry_pick_current:
    ...
    # Get first parent.
    parent = None
    first_cl = ordered_cls[0]
    origin, upstream_branch_ref = first_cl.FetchUpstreamTuple(
        first_cl.GetBranch())
    upstream_branch = scm.GIT.ShortBranchName(upstream_branch_ref)
    if origin == '.':
    ...
    if not parent:
      # upstream is either the root of the tree or a local branch that is empty
      # and has never been uploaded.
      parent = first_cl.GetCommonAncestorWithUpstream()
...
def _UploadAllPrecheck(options, orig_args):
  ...
  while True:
    ...
    end_commit = RunGit(['rev-parse', cl.branchref]).strip()

    diff = RunGitSilent(
        ['diff',
         '%s..%s' % (upstream_last_upload or base_commit, end_commit)])
    if diff:
      cls.append(cl)
      ...
      first_pass = False
      DieWithError('Branch %s has nothing to commit' % cl.branch)
