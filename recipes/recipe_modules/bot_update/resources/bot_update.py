...
# Regular expression to extract default branch from the result of running
# `git ls-remote --symref origin HEAD`.
DEFAULT_BRANCH_RE = re.compile(r'ref:\s+refs/heads/(\S*)\s+HEAD')
...
def get_target_pin(solution_name, git_url, revisions):
  ...
  _, revision = _get_target_branch_and_revision(
      solution_name, git_url, revisions, None)
...
def _git_checkout(sln, sln_dir, revisions, refs, no_fetch_tags, git_cache_dir,
  ...
  while True:
    try:
      ...
      remote_head = git('ls-remote', '--symref', url, 'HEAD')
      default_branch = DEFAULT_BRANCH_RE.match(remote_head).group(1)
