...
def _DeepFormat(root, vars_dict):
  def _Format(node):
    if isinstance(node, basestring):
      return node.format(**vars_dict)
    elif isinstance(node, dict):
      return {
          k.format(**vars_dict): _Format(v)
          for k, v in node.iteritems()
      }
    elif isinstance(node, list):
      return [_Format(elem) for elem in node]
    elif isinstance(node, tuple):
      return tuple(_Format(elem) for elem in node)
    else:
      return node
  return _Format(root)
      ...
      if not isinstance(target, ast.Name):
        raise ValueError(
            'invalid assignment: target should be a name (file %r, line %s)' % (
                filename, getattr(node, 'lineno', '<unknown>')))
      value = _gclient_eval(node.value, vars_dict,
...
def Parse(content, expand_vars, validate_syntax, filename, vars_override=None):
  ...
  return _DeepFormat(local_scope, vars_dict)
