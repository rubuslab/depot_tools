...
def CMDcherry_pick(parser, args):
    ...
    if not options.host or not options.branch:
        parser.error('Both host and branch are required.')
    if not args:
    ...
    for change_id, orig_message in change_ids_to_message.items():
        ...
        new_change_info = gerrit_util.CherryPick(options.host,
                                                 change_id,
                                                 options.branch,
                                                 message=message)
        new_change_id = new_change_info['change_id']
        new_change_num = new_change_info['_number']
        new_change_url = gerrit_util.GetChangePageUrl(options.host,
                                                      new_change_num)
        ...
        if parent_change_num:
            try:
                ...
                gerrit_util.RebaseChange(options.host, new_change_id,
                                         parent_change_num)
            except gerrit_util.GerritError as e:
                parent_change_url = gerrit_util.GetChangePageUrl(
                    options.host, parent_change_num)
                ...
                print('Once resolved, you can continue the CL chain with '
                      '`--parent-change-num` and specify which change the '
