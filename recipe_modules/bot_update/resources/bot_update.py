...
def apply_gerrit_ref(gerrit_repo, gerrit_ref, root, gerrit_reset,
  ...
  # Speculative fix: prior bot_update run with Rietveld patch may leave git index
  # with unmerged paths. bot_update calls 'checkout --force xyz' thus ignoring
  # such paths, but potentially never cleaning them up. The following command
  # will do so. See http://crbug.com/692067.
