...
class GcsDependency(Dependency):
    ...
    def __init__(self, parent, name, bucket, path, sha256sum, custom_vars,
                 should_process, relative, condition):
        self.bucket = bucket
        self.path = path
    ...
    def IsDownloadNeeded(self, output_dir, output_file):
        ...
        new_file_hash = self.GetSha1(self.path)
    ...
    def DownloadGoogleStorage(self):
        ...
        if os.getenv('GCLIENT_TEST') == '1':
            ...
            calculated_sha256sum = subprocess2.check_output(
                checksum_cmd).decode('utf-8').split(' ')[0]
