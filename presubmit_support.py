    ...
    def GetDiff(self, path, local_root):
        ...
        if self._diffs_by_file == None:
            ...
            unified_diff = scm.GIT.GenerateDiff(
                local_root, files=[], full_move=True, branch=self._upstream
            )
...
class Change(object):
    ...
    def AffectedTestableFiles(self, include_deletes=None, **kwargs):
        ...
        if include_deletes is not None:
            warn(
                "AffectedTeestableFiles(include_deletes=%s)"
                " is deprecated and ignored" % str(include_deletes),
                category=DeprecationWarning,
                stacklevel=2,
            )
...
def _MergeMasters(masters1, masters2):
    ...
    result = {}
    for master, builders in itertools.chain(masters1.items(), masters2.items()):
...
def DoPresubmitChecks(
    ...
    try:
        ...
        if committing:
            sys.stdout.write(
                "Running %s presubmit commit checks ...\n" % python_version
            )
        else:
            sys.stdout.write(
                "Running %s presubmit upload checks ...\n" % python_version
            )
        start_time = time_time()
        presubmit_files = ListRelevantPresubmitFiles(
        ...
        if not should_prompt and not presubmits_failed:
            ...
            sys.stdout.write(
                "There were %s presubmit warnings. " % python_version
            )
            if may_prompt:
...
def main(argv=None):
    ...
    parser.add_argument(
        "--json_output", help="Write presubmit errors to json output."
    )
    parser.add_argument(
        "--all_files",
        action="store_true",
