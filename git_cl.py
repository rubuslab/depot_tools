...
def _fetch_from_buildbucket(http, method, url, data=None):
  """Makes requests to buildbucket and returns unparsed json content."""
  headers = {
...
def _parse_bucket(bucket):
  if '/' in bucket:
    return tuple(bucket.split('/'))
  if bucket.startswith('luci.'):
    bucket = bucket[len('luci.'):]
    return tuple(bucket.split('.', 1))
  # Legacy buckets: Assume prefix is also the project name.
  if '.' in bucket:
    print('WARNING please specify buckets as <project>/<bucket> or '
          'luci.<project>.<bucket>.')
...
def _trigger_try_jobs(auth_config, changelist, buckets, options, patchset):
  ...
  buildbucket_endpoint = (
      'https://%s/prpc/buildbucket.v2.Builds/Batch' % options.buildbucket_host)

  batch_request = {'requests': requests}
  batch_response = _fetch_from_buildbucket(
      http, 'POST', buildbucket_endpoint,
      data=json.dumps(batch_request, sort_keys=True))

  responses = json.loads(batch_response[4:]).get('responses', [])

  errors = [
      '  ' + response['error']['message']
      for response in responses if 'error' in response
  ]
  if errors:
...
def fetch_try_jobs(auth_config, changelist, buildbucket_host,
  ...
  while True:
    ...
    content = json.loads(_fetch_from_buildbucket(http, 'GET', url))
