...
def RunSteps(api):
    ...
    if api.properties.get("patch_text"):
        api.step(
            ...
            [
                "echo",
                str(api.tryserver.get_footers(api.properties["patch_text"])),
            ],
        )
        api.step(
            "patch_text test",
    ...
    if api.tryserver.gerrit_change:
        ...
        assert (
            api.tryserver.gerrit_change_repo_host == "chromium.googlesource.com"
        )
        assert api.tryserver.gerrit_change_repo_project == "chromium/src"
        assert (
            api.tryserver.gerrit_change_fetch_ref == "refs/changes/27/91827/1"
        )
...
def GenTests(api):
    ...
    yield (
        ...
        + api.step_data(
            "parse description (2)", api.json.output({"Foo": ["bar"]})
        )
    )
