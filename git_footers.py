...
def get_position(footers):
  ...
  Returns:
    A tuple containing (ref,pos) of the commit, or (None,1) if it's the first
    commit.
...
def main(args):
  ...
  if opts.key:
    ...
    parent_ref, parent_pos = get_position(footers)
    if (not parent_ref):
      raise ValueError("Cr-Commit-Position not found")
    print('%s@{#%s}' % (parent_ref, parent_pos or '?'))
  elif opts.position_ref:
    parent_ref, _ = get_position(footers)
    if not parent_ref:
      raise ValueError("Cr-Commit-Position not found")
    print(parent_ref)
  elif opts.position_num:
    _,parent_pos = get_position(footers)
    print(parent_pos)
  elif opts.json:
