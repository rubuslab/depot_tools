...
def _run_test(with_subprocess=True):
  ...
  no_op = lambda s: s
  to_cr = lambda s: s.replace('\n', '\r')
  to_crlf = lambda s: s.replace('\n', '\r\n')
  def wrapper(test):
    def inner(self):
      for subp in subps:
        # universal_newlines = False
        test(self, no_op, TEST_COMMAND, False, subp)
        test(self, to_cr, TEST_COMMAND + ['--cr'], False, subp)
        test(self, to_crlf, TEST_COMMAND + ['--crlf'], False, subp)
...
class SmokeTests(unittest.TestCase):
  ...
  def test_check_output_no_stdout(self):
    try:
      subprocess2.check_output(TEST_COMMAND, stdout=subprocess2.PIPE)
      self.fail()
    except ValueError:
      pass
    ...
    if (sys.version_info[0] * 10 + sys.version_info[1]) >= 27:
      ...
      try:
        ...
        subprocess.check_output(TEST_COMMAND, stdout=subprocess.PIPE)
        self.fail()
      except ValueError:
        pass
  ...
  @_run_test(with_subprocess=False)
  def test_stdin_unicode(self, c, cmd, un, subp):
    stdin = u'0123456789'
    res = subprocess2.communicate(
        cmd + ['--read'],
        stdin=stdin,
  ...
  def test_stdin_void(self):
    res = subprocess2.communicate(TEST_COMMAND + ['--read'], stdin=subprocess2.VOID_INPUT)
    self._check_res(res, None, None, 0)
