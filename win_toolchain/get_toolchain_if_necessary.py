...
def UsesToolchainFromFile():
  url = os.environ.get('DEPOT_TOOLS_WIN_TOOLCHAIN_BASE_URL', '')
  return bool(int(os.environ.get('USE_LOCAL_ZIP', 0))) or os.path.isdir(url)


def UsesToolchainFromHttp():
  return os.environ.get('DEPOT_TOOLS_WIN_TOOLCHAIN_BASE_URL',
                        '').startswith('http://')
...
def DownloadUsingHttp(filename):
  ...
  if not base_url.startswith('http://'):
    sys.exit('DEPOT_TOOLS_WIN_TOOLCHAIN_BASE_URL environment variable'
             'needs to point to a \'http\' URL')
