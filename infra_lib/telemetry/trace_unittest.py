...
def test_span_to_capture_keyboard_interrupt_as_decorator() -> None:
    ...
    provider = trace.ChromiumTracerProvider(trace_sdk.TracerProvider())
...
def test_span_to_capture_failed_packages_in_context() -> None:
    """Test span can capture failed_packages in context."""
    exporter = SpanExporterStub()
    provider = trace.ChromiumTracerProvider(trace_sdk.TracerProvider())
    provider.add_span_processor(
        export_sdk.BatchSpanProcessor(span_exporter=exporter))
    ...
    with contextlib.suppress(ExceptionWithFailedPackages):
        with tracer.start_as_current_span("test-span"):
            raise ExceptionWithFailedPackages(msg="testing",
                                              pkgs=["dev-python/boto"])

    provider.shutdown()

    assert len(exporter.spans) == 1
    assert exporter.spans[0].status.status_code == trace_api.StatusCode.ERROR
    assert (exporter.spans[0].status.description ==
...
def test_span_to_capture_failed_packages_as_decorator() -> None:
    """Test span can capture failed_packages as decorator."""
    exporter = SpanExporterStub()
    provider = trace.ChromiumTracerProvider(trace_sdk.TracerProvider())
    provider.add_span_processor(
        export_sdk.BatchSpanProcessor(span_exporter=exporter))
