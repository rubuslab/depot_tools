...
def _get_gcs_object_list_ast(package_ast: ast.Dict) -> ast.List:
  ...
  for key, value in zip(package_ast.keys, package_ast.values):
    if not isinstance(key, ast.Constant):
      continue
    if key.value == 'dep_type' and isinstance(value, ast.Constant) and value.value == 'gcs':
      is_gcs = True
...
def _replace_ast(destination: str, dest_ast: ast.Module, source: str,
  ...
  if source_ast.lineno == source_ast.end_lineno:
    ...
    result += source_lines[source_ast.lineno - 1][source_ast.col_offset:source_ast.end_col_offset]
  else:
    # Multiline content. The first line and the last line of source AST should
    # be partially copied.

    # Partially copy the first line of source AST.
    result += source_lines[source_ast.lineno - 1][source_ast.col_offset:] + '\n'
    ...
    result += source_lines[source_ast.end_lineno - 1][:source_ast.end_col_offset]
...
def main():
  ...
  if not args.package or len(args.package) < 1:
    parser.error(
        'No packages specified to roll, aborting...'
    )
