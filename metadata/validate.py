...
def validate_file(filepath: str,
  ...
  if not os.path.exists(filepath):
    result = vr.ValidationError(f"Metadata file '{filepath}' not found.")
...
def check_file(filepath: str,
  ...
  for result in validate_file(filepath, repo_root_dir):
    ...
    postscript = _CHROMIUM_METADATA_POSTSCRIPT
    if result.get_tag("reason") == "file not found":
      postscript = ""
    message = result.get_message(postscript=postscript, width=60)

  # TODO(aredulla): Actually distinguish between validation errors and warnings.
  # The quality of metadata is currently being uplifted, but is not yet
  # guaranteed to pass validation. So for now, all validation results will be
  # returned as warnings so that changes are not blocked by metadata presubmits.
  # if result.is_fatal():
