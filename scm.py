...
class GIT(object):
  ...
  #
  # This cache speeds up all `git config ...` operations by only running a single
  # subcommand, which can greatly accelerate things like git-map-branches.
  _CONFIG_CACHE: Dict[str, Dict[str, List[str]]] = {}

  @staticmethod
  def _load_config(cwd) -> Dict[str, List[str]]:
    cached: Optional[Dict[str, List[str]]] = GIT._CONFIG_CACHE.get(cwd, None)
    if cached is None:
      cached = {}
      try:
        allConfig = GIT.Capture(['config', '--list'], cwd=cwd, strip_out=False)
  ...
  def YieldConfigRegexp(cwd, pattern):
    ...
    for name, values in GIT._load_config(cwd):
      if p.match(name):
        for value in values:
          yield name, value
  ...
  @staticmethod
  def SetConfig(cwd, key, value=None, *, value_pattern=None, all=False, scope='local'):
    GIT._clear_cache(cwd)
    if value is None:
      args = ['config', f'--{scope}', '--unset'+('-all' if all else ''), key]
    else:
