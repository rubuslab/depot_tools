...
class BotUpdateApi(recipe_api.RecipeApi):
    ...
    def __init__(
        self, properties, deps_revision_overrides, fail_patch, *args, **kwargs
    ):
    ...
    def _get_bot_update_env(self):
        ...
        env = {
            ...
            "GIT_TRACE2_EVENT": self.m.path.join(
                self._trace_dir, "trace2-event"
            ),
            "GIT_TRACE_CURL": self.m.path.join(self._trace_dir, "trace-curl"),
            "GIT_TRACE_CURL_NO_DATA": 1,
            "GIT_TRACE_PACKET": self.m.path.join(
                self._trace_dir, "trace-packet"
            ),
            "GIT_BACKENDINFO": 1,
            "GIT_DAPPER_TRACE": 1,
    ...
    def _upload_traces(self):
        with self.m.step.nest("upload git traces") as presentation:
            ...
            try:
                ...
                self.m.gsutil(
                    ["cp", zip_path, "gs://chrome-bot-traces"], name="upload"
                )
    ...
    def ensure_checkout(
        ...
        **kwargs
        ...
        if patch:
            ...
            if patch_refs:
                flags.extend(
                    ["--patch_ref", patch_ref] for patch_ref in patch_refs
                )
        ...
        if not ignore_input_commit and in_commit_rev:
            ...
            in_commit_repo_path = in_commit_repo_path.replace(
                self.m.path.sep, "/"
            )
            ...
            if (
                ...
                if update_presentation:
                    ...
                    for (
                        prop_name,
                        prop_value,
                    ) in self.last_returned_properties.items():
                ...
                if (
                    step_result.exc_result.was_cancelled
                ...
                if (
                    set_output_commit
                    and "got_revision" in self._last_returned_properties
                    ...
                    in_rev = self.m.gclient.resolve_revision(
                        revisions.get(out_solution)
                    )
                    if not in_rev:
                        in_rev = "HEAD"
                    if got_revision_cp:
    ...
    def _resolve_fixed_revisions(self, bot_update_json):
        ...
        for name in bot_update_json.get("fixed_revisions", {}):
            rev_properties = self.get_project_revision_properties(name)
            if rev_properties and bot_update_json["properties"].get(
                rev_properties[0]
            ):
    ...
    def get_project_revision_properties(
        self, project_name, gclient_config=None
    ):
    ...
    def deapply_patch(self, bot_update_step):
        ...
        rev_property = self.get_project_revision_properties(
            first_solution_name
        )[0]
