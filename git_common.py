...
def rebase(parent, start, branch, abort=False, allow_gc=False):
  ...
  Args:
    ...
    allow_gc - If True, doesn't set "-c gc.auto 0" on the rebase call.
               Usually if you're doing a series of rebases, you'll only want to
               run a single gc pass at the end of all the rebase activity.
  ...
  try:
    args = [
      '-c', 'gc.auto', '1' if allow_gc else '0',
      'rebase',
