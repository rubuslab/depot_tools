...
def _git_set_branch_config_value(key, value, branch=None, **kwargs):
  ...
  If None is given for branch, the value will be unset, otherwise
  it will be set.

  A valid, though not necessarily existing, branch must be provided,
  otherwise the currently checked out branch is used.
  """
  if not branch:
    branch = GetCurrentBranch()
