...
def DepotToolsPylint(input_api, output_api):
  ...
  if os.path.exists('.gitignore'):
    with open('.gitignore') as fh:
      ...
      block_list.extend(
          [fnmatch.translate(l) for l in lines if l and not l.startswith('#')])
  ...
  return input_api.canned_checks.GetPylint(input_api,
                                           output_api,
...
def CommonChecks(input_api, output_api, tests_to_block_list, run_on_python3):
  ...
  tests.extend(
      input_api.canned_checks.GetUnitTestsInDirectory(
          ...
          output_api,
          'tests',
          allowlist=tests_to_allow_list,
          blocklist=tests_to_block_list,
...
def CheckChangeOnUpload(input_api, output_api):
  ...
  return CommonChecks(input_api, output_api, tests_to_block_list, False)
