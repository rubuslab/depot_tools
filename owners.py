...
class Database(object):
  ...
  def files_not_covered_by(self, files, reviewers):
    ...
    #print('type is %s' % type(files))
    otype = type(files)
    if type(files) == type(''):
      files = files.replace(os.sep, '//')
    elif type(files) == type([]):
      files = [f.replace(os.sep, '/') for f in files]
    elif type(files) == type(set):
      files = set([f.replace(os.sep, '/') for f in files])
    newtype = type(files)
    if otype != newtype:
      print("Unexpected type is %s, was %s" % (type(files), otype))
    ##  print files
    #  raise Exception()
    self._check_paths(files)
  ...
  def _fnmatch(self, filename, pattern):
    ...
    # Make sure backslashes are never used. The regex expressions generated by
    # fnmatch.translate don't handle matching slashes to backslashes.
    assert filename.count('\\') == 0, 'Backslashes found in %s' % filename
    assert pattern.count('\\') == 0, 'Backslashes found in %s' % filename
