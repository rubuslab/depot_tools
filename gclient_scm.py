...
class GitWrapper(SCMWrapper):
    ...
    def set_config(f):
        def wrapper(*args):
            ...
            if os.path.exists(os.path.join(args[0].checkout_path, '.git')):
                ...
                config = scm.GIT.GetAllConfig(args[0].checkout_path)

                if 'diff.ignoresubmodules' not in config or config['diff.ignoresubmodules'] != 'dirty':
                    scm.GIT.SetConfig(args[0].checkout_path, 'diff.ignoreSubmodules', 'dirty')

                if 'fetch.recurseSubmodules' not in config or config['fetch.recurseSubmodules'] != 'off':
                    scm.GIT.SetConfig(args[0].checkout_path, 'fetch.recurseSubmodules', 'off')

                if 'push.recurseSubmodules' not in config or config['push.recurseSubmodules'] != 'off':
                    scm.GIT.SetConfig(args[0].checkout_path, 'push.recurseSubmodules', 'off')
    ...
    """
        ...
        # let that through).
        current_url = scm.GIT.GetConfig(self.checkout_path, 'remote.%s.url' % self.remote)
        return_early = False
        ...
        if (strp_current_url.rstrip('/') != strp_url.rstrip('/')
                and url != 'git://foo' and
                scm.GIT.GetConfig(self.checkout_path,
    ...
    if requested."""
        if options.force or options.reset:
            try:
                scm.GIT.SetConfig(self.checkout_path, 'remote.%s.fetch' % self.remote, value_pattern=self.filter)
                scm.GIT.SetConfig(self.checkout_path,
                                  'remote.%s.fetch' % self.remote,
                                  '+refs/heads/*:refs/remotes/%s/*' % self.remote,
                                  value_pattern=self.filter)
            except subprocess2.CalledProcessError as e:
