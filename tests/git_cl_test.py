...
class TestGitClBasic(unittest.TestCase):
  ...
  @mock.patch('git_cl.Changelist.GetStatus')
  def test_get_cl_statuses(self, *_mocks):
    changes = [git_cl.Changelist() for _ in range(7)]
    statuses = [
        'waiting',
        'reply',
        'unsent',
    ...
    git_cl.Changelist.GetStatus.side_effect = statuses

    actual = list(git_cl.get_cl_statuses(changes, True))
    self.assertEqual(list(zip(changes, statuses)), actual)
  ...
  def test_get_cl_statuses_timeout(self, *_mocks):
    ...
    pool.imap_unordered().__iter__ = mock.Mock()
    it = pool.imap_unordered().__iter__()
    it.next.side_effect = [
        (changes[0], 'lgtm'),
  ...
  def test_get_cl_statuses_not_finegrained(self, _mock):
    ...
    actual = list(git_cl.get_cl_statuses(changes, False))
    self.assertEqual([(changes[0], 'waiting'), (changes[1], 'error')], actual)
