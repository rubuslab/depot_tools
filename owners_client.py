...
class OwnersClient(object):
  ...
  def ScoreOwners(self, paths, iterations=1):
    ...
    for _ in range(iterations):
      ...
      for owner, score in score_by_owner.items():
        total_score[owner] = total_score.get(owner, 0) + score

    # Sort owners by their score. Randomize order of owners with same score.
    return sorted(total_score, key=lambda o: (total_score[o], random.random()))
