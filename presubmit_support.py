...
class InputApi(object):
  ...
  def __init__(self, change, presubmit_path, is_committing,
    ...
    if gerrit_obj:
      if gerrit_obj.host:
        host = gerrit_obj.host
      if gerrit_obj.project:
        project = gerrit_obj.project

    branch = ''
    try:
      branch = subprocess.check_output(
          ['git', 'name-rev', '--name-only', change.UpstreamBranch()],
          cwd=change.RepositoryRoot()).strip()
...
def _parse_gerrit_options(parser, options):
  ...
  if options.gerrit_url:
    gerrit_obj = GerritAccessor(
        urlparse.urlparse(options.gerrit_url).netloc,
