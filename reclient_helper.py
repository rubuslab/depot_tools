...
def find_cache_dir(tmp_dir):
  ...
  tmp_dir should be a build specific temp directory, ideally within the out directory.
  
  If this is called from within a gclient checkout then the cache dir will be:
  <gclient_root>/.reproxy_cache/md5(tmp_dir)/
  If this is not called from within a gclient checkout then the cache dir will be:
  tmp_dir/cache
  """
  gclient_root = gclient_paths.FindGclientRoot(os.getcwd())
  if gclient_root:
    return os.path.join(gclient_root, '.reproxy_cache', hashlib.md5(tmp_dir))
