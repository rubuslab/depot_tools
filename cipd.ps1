...
$user_agent = "depot_tools/$(&{git -C $myPath rev-parse HEAD})"
...
if ({& $client selfupdate -http-user-agent "$user_agent" -version "$cipdClientVer"} -ne 0) {
    Write-Host "selfupdate failed: " -ForegroundColor Red -NoNewline
    Write-Host "run ``$client selfupdate -http-user-agent "$user_agent/manual" -version $cipdClientVer`` to diagnose`n" -ForegroundColor White
}

& $client @args -http-user-agent "$user_agent"
exit $LASTEXITCODE
