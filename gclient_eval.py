...
def AddVar(gclient_dict, var_name, value):
  ...
  if 'vars' not in gclient_dict:
    raise KeyError("vars dict is not defined. This is not yet supported.")
  ...
  # We will try to match the new var's indentation to the next variable, or
  # set it to 2 if no vars are defined.
  if gclient_dict['vars']:
