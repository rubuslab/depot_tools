    ...
    def _launch_sso_helper(cls) -> SSOInfo:
        ...
        with tempdir() as tdir:
            ...
            with open(tf, mode='w') as stderr_file:
                ...
                # 2. writes config to stdout, then closes stdout (thus sending EOF
                #    to us, allowing sys.stdout.read() to complete).
                # 3. waits for stdin to close && deletes files on disk (which is why
                #    we make sys.stdin a PIPE instead of closing it outright).
                #
                # Notably: the actual proxy service continues running in the
                # background after this process terminates.
                with subprocess2.Popen(cmd,
                    ...
                    # if the process ended, then something is wrong
                    retcode = proc.poll()
                    if not stdout_data:
                        # if stdout was closed without any data, we need to wait
                        # for end-of-process here and hope for an error message
                        # - the poll above is racy in this case (we could see
                        # stdout EOF but the process may not have quit yet).
