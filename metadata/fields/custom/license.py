...
def process_license_value(value: str) -> List[Tuple[str, bool]]:
  """Process the value of a license field into separate licenses."""
...
class LicenseField(field_types.MetadataField):
  ...
  def validate(self, value: str) -> Union[vr.ValidationResult, None]:
    ...
    """
    not_allowlisted = []
    for license, allowed in process_license_value(value):
    ...
    if (re.search(_PATTERN_VERBOSE_DELIMITER, value)
        and util.VALUE_DELIMITER not in value):
      return vr.ValidationWarning(
          f"{self._name} should use '{util.VALUE_DELIMITER}' to delimit "
          "values.")
