  ...
  def _UpdateWithExternalChanges(self):
    ...
    if local_base != external_base:
      ...
      if git_common.upstream(branch):
        ...
        try:
          retcode, _ = RunGitWithCode(['tag', tagname, external_base])
          success = retcode == 0
          success = success and git_reparent_branch.main([
            tagname,
          ]) == 0
        ...
        if not success:
          ...
          # tags run into "Upload upstream branch git-cl-upload-4082288-baz-2-base first."
          print()
