...
class Changelist(object):
  ...
  def GetCommonAncestorWithUpstream(self):
    ...
    if not scm.GIT.IsValidRevision(upstream_branch):
  ...
  def GetIssue(self):
    ...
    if self.issue is None and not self.lookedup_issue:
      self.issue = self._GitGetBranchConfigValue('gerritissue')
  ...
  def GetPatchset(self):
    ...
    if self.patchset is None and not self.lookedup_patchset:
      self.patchset = self._GitGetBranchConfigValue('gerritpatchset')
  ...
  def SetPatchset(self, patchset):
    ...
    self._GitSetBranchConfigValue('gerritpatchset', str(self.patchset))
  ...
  def SetIssue(self, issue=None):
    ...
    if issue:
      issue = int(issue)
      self._GitSetBranchConfigValue('gerritissue', str(issue))
      self.issue = issue
      codereview_server = self.GetCodereviewServer()
      if codereview_server:
        self._GitSetBranchConfigValue('gerritserver', codereview_server)
    else:
  ...
  def GetCodereviewServer(self):
    if not self._gerrit_server:
      ...
      if self.GetIssue() and self.GetBranch():
        self._gerrit_server = self._GitGetBranchConfigValue('gerritserver')
  ...
  if options.reverse:
    ...
    for branch in branches:
      issue = git_config.get(
          'branch.%s.gerritissue' % scm.GIT.ShortBranchName(branch))
      if issue:
...
def CMDcheckout(parser, args):
  ...
  for key, issue in [x.split() for x in output.splitlines()]:
    if issue == target_issue:
      branches.append(re.sub(r'branch\.(.*)\.gerritissue', r'\1', key))
