...
class CipdDependency(Dependency):
    ...
    def run(self, revision_overrides, command, args, work_queue, options,
        ...
        # TODO(b/349643421)GCS dependencies do not need to run during runhooks.
        # Note, for a GCSDependency we also return early for the `revinfo`
        # command, however, this currently breaks testRevInfoActual() in
        # gclient_cipd_smoketest.py. b/349699772 may be relevant.
        if command == 'runhooks':
            return
        if not self.should_process:
            return
