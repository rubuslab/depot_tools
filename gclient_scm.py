...
class GitWrapper(SCMWrapper):
  ...
  def _FetchAndReset(self, ref, remote_ref, revision, file_list, options):
    ...
    self._Scrub(revision or ref, options)
  ...
  def update(self, options, args, file_list):
    ...
    # once we call _AutoFetchRevision.
    if ':' in revision:
    ...
    if ref:
      ...
      remote_ref = ''.join(remote_ref) if remote_ref else ref
    ...
    if current_type == 'detached':
      ...
      if not (options.force or options.reset):
        self._CheckClean(ref or revision)
      self._CheckDetachedHead(ref or revision, options)
      ...
      if not printed_path:
        self.Print('_____ %s at %s' % (self.relpath, ref or revision),
                   timestamp=False)
      ...
      upstream_branch = ref or revision or self.remote
    ...
    if self._GetCurrentBranch() is None:
      ...
      self.Print(
        ('Checked out %s to a detached HEAD. Before making any commits\n'
         ...
         'create a new branch for your work.') % (revision or ref, self.remote))
  ...
  def _UpdateBranchHeads(self, options, ref, remote_ref, fetch=False):
    ...
    if hasattr(options, 'with_tags') and options.with_tags:
      ...
      self._Run(config_cmd, options)
      need_fetch = True
    # Make sure we fetch the ref if we're asked to sync to one.
