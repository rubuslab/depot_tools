...
class Dependency(gclient_utils.WorkItem, DependencySettings):
  ...
  def ParseDepsFile(self):
    ...
    if 'deps_os' in local_scope:
      ...
      if target_os_list and not self._get_option('ignore_deps_os', False):
    ...
    if 'hooks_os' in local_scope and target_os_list:
      ...
      if not self._get_option('ignore_deps_os', False):
...
def CMDsync(parser, args):
  ...
  parser.add_option('--deps', dest='deps_os', metavar='OS_LIST',
                    help='override deps for the specified (comma-separated) '
                         'platform(s); \'all\' will process all deps_os '
                         'references')
  parser.add_option('--ignore-deps-os', action='store_true')
