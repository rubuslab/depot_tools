...
class GclientApi(recipe_api.RecipeApi):
    ...
    def __call__(self, name, cmd, infra_step=True, **kwargs):
        ...
        with self.m.context(env=env, env_suffixes=env_suffixes):
            ...
            return self.m.step(
                prefix + name, cmd, infra_step=infra_step, **kwargs
            )
    ...
    def sync(self, cfg, extra_sync_flags=None, **kwargs):
        ...
        for i, s in enumerate(cfg.solutions):
            ...
            if s.revision is not None and s.revision != "":
                ...
                if fixed_revision:
                    revisions.extend(
                        ["--revision", "%s@%s" % (s.name, fixed_revision)]
                    )
        ...
        for name, revision in sorted(cfg.revisions.items()):
            ...
            if fixed_revision:
                revisions.extend(
                    ["--revision", "%s@%s" % (name, fixed_revision)]
                )
        ...
        try:
            ...
            if result.json.output is not None:
                ...
                for propname, path in sorted(
                    ...
                    if info:
                        result.presentation.properties[propname] = info[
                            "revision"
                        ]
        ...
        for prop, custom_var in cfg.parent_got_revision_mapping.items():
            ...
            if val:  # pragma: no cover
                ...
                if custom_var is None:
                    ...
                    if (
                        cfg.solutions[0].revision is None or override
                    ):  # pragma: no cover
                        cfg.solutions[0].revision = val
                else:
                    if (
                        custom_var not in cfg.solutions[0].custom_vars
                        or override
                    ):
    ...
    def checkout(
        ...
        **kwargs,
        ...
        self(
            "setup",
            ["config", "--spec", self.config_to_pythonish(cfg)],
            **kwargs,
        )
        ...
        try:
            sync_step = self.sync(
                cfg, extra_sync_flags=extra_sync_flags, **kwargs
            )
    ...
    def roll_deps(
        ...
        deps_content = self.m.file.read_text(
            "Read DEPS file", deps_path, test_data
        )
