...
class Changelist(object):
  ...
  def GetRemoteDescription(self, pretty=False):
  ...
  def GetChange(self, upstream_branch, author, local_description=False):
    ...
    if issue and not local_description:
      description = self.GetRemoteDescription()
    else:
  ...
  def UpdateRemoteDescription(self, description, force=False):
  ...
  def CMDUploadChange(self, options, git_diff_args, custom_cl_base, change):
    ...
    if options.squash:
      ...
      if self.GetIssue():
        ...
        message = self.GetRemoteDescription()
        if not message:
          DieWithError(
...
def CMDstatus(parser, args):
  ...
  if not options.fast:
    ...
    print(cl.GetRemoteDescription(pretty=True))
  return 0
...
def CMDdescription(parser, args):
  ...
  description = ChangeDescription(cl.GetRemoteDescription())
  ...
  if options.new_description:
    ...
    description.prompt()
  if cl.GetRemoteDescription().strip() != description.description:
    cl.UpdateRemoteDescription(description.description, force=options.force)
  return 0
