...
class GclientTest(trial_dir.TestCase):
    ...
    def testParseDepsFile_FalseShouldSync_WithCustoms(self):
        ...
        six.assertCountEqual(
            self, expected_dep_names, [d.name for d in sol.dependencies]
        )
        six.assertCountEqual(
            self, expected_hook_names, [h.name for h in sol._deps_hooks]
        )
    ...
    def testPosixpathCipdSubdirOnWindows(self):
        with mock.patch("os.path", new=ntpath), mock.patch(
            "os.sep", new=ntpath.sep
        ):
            self._testPosixpathImpl()
