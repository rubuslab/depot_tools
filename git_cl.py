...
class ChangeDescription(object):
  ...
  def update_with_git_number_footers(self, parent_hash, parent_msg, dest_ref):
    ...
    for i, kv in enumerate(parsed_footers):
      k, v = kv
      if k.startswith('Cr-'):
        parsed_footers[i] = (k.replace('Cr-', 'Cr-Original-'), v)

    # Add Position footer based on the parent.
  ...
  try:
    ...
    if cmd == 'land':
      ...
      if ShouldGenerateGitNumberFooters():
        ...
        # TODO(tandrii): what abotu pending? Worst case we get double footers in
        # Gnumbd?
