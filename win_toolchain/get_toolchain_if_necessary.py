...
def ToolchainDir():
  path = os.environ.get('DEPOT_TOOLS_WIN_TOOLCHAIN_BASE_URL', '')
...
def UsesToolchainFromFile():
  return os.path.isdir(ToolchainDir())
...
def DownloadUsingHttp(filename):
  ...
  base_url = os.environ['DEPOT_TOOLS_WIN_TOOLCHAIN_BASE_URL']
...
def DoTreeMirror(target_dir, tree_sha1):
  ...
  if UsesToolchainFromFile():
    temp_dir = None
    local_zip = os.path.join(ToolchainDir(), tree_sha1 + '.zip')
