...
class OSXSDKTestApi(recipe_test_api.RecipeTestApi):
  ...
  def osx_version(
    ...
    the Xcode SDK version from it's internal table.
  ...
  def pick_sdk_version(self,
    ...
    Recipes used on builder which configure the XCode version via properties
    ...
    version (which is mockable with the `osx_version` method above).
