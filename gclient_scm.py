...
class GitWrapper(SCMWrapper):
  ...
  def apply_patch_ref(self, patch_repo, patch_rev, target_rev, options,
    ...
    The patch ref is given by |patch_repo|@|patch_rev|, and the current revision
    is |base_rev|.

    In order to find what commits constitute the patch, we need a |target_rev|
    that has the parent of the patch commits as an ancestor. We then find the
    parent as the merge base between |target_rev| and |patch_rev|.
    |target_ref| is usually the branch that the |patch_rev| was uploaded against
    (e.g. 'refs/heads/master'), but this is not required.

    Graphically, it looks like this:

     ... -> merge_base -> [possibly already landed commits] -> target_rev
                       \
                        -> [possibly not yet landed dependent CLs] -> patch_rev

    Next, we cherry-pick the commits |merge_base..patch_rev| on top of the
    current HEAD, which we denote with |base_rev|.
    |base_rev| is typically an ancestor of |target_rev|, but this is not
    required.
    ...
    self.Print(
        'Revision to patch is %r @ %r. Will apply patch on top of %r. '
        '%r will be used to compute the merge base. Current dir is %r' % (
            patch_repo, patch_rev, base_rev, target_rev, self.checkout_path))
    ...
    try:
      if not options.rebase_patch_ref:
        ...
        # Find the merge-base between the target_rev and patch_rev to find out
        # the changes we need to cherry-pick on top of base_rev.
        merge_base = self._Capture(['merge-base', target_rev, patch_rev])
        self.Print('Merge base of %s and %s is %s' % (
            target_rev, patch_rev, merge_base))
      ...
      self.Print(
        'Revision to patch is %r @ %r. Tried to apply patch on top of %r. '
        '%r was used to compute the merge base. Current dir is %r' % (
            patch_repo, patch_rev, base_rev, target_rev, self.checkout_path))
