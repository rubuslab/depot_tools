...
class GSUtilApi(recipe_api.RecipeApi):
  ...
  def disable_multiprocessing_on_mac(self):
    if not self.m.platform.is_mac:
      yield
      return

    # BOTO_CONFIG can only point to one file; BOTO_PATH can point to multiple.
    # If BOTO_CONFIG is set, BOTO_PATH is ignored. If neither are set, gsutil
    # looks at default locations (/etc/boto.cfg and ~/.boto). So try
    # preserving that ordering, but give up in the default case just to avoid a
    # headache.
    if not self._boto_config_path or not self._boto_path:
      yield
      return
    custom_boto_path = self.m.path.mkstemp(suffix='_boto.txt')
    contents = textwrap.dedent('''
        # Generated by $depot_tools.recipe_modules.gsutil
        [GSUtil]
        parallel_process_count = 1
    ''')
    self.m.file.write_text('write temp Boto file', custom_boto_path, contents)
    if self._boto_config_path:
      custom_boto_path = self._boto_config_path + ':' + custom_boto_path
    elif self._boto_path:
      custom_boto_path = self._boto_path + ':' + custom_boto_path
    with self.m.context(env={'BOTO_PATH': custom_boto_path}):
      yield
