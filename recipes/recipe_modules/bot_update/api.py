...
class BotUpdateApi(recipe_api.RecipeApi):
  ...
  def _get_associated_solution(self, commit, solutions):
    ...
    The commit must be a self.m.buildbucket.common_pb2.GitilesCommit and
    solutions must be a non-empty list of gclient solutions.

    If multiple gclient solutions match the commit's repo (e.g. multiple
    solutions specified same URL), returns the first match.

    Raises an InfraFailure if an invalid repo is specified in the commit.
    """
  ...
  def ensure_checkout(self, gclient_config=None, suffix=None,
    ...
    if (input_commit.id or input_commit.ref) and cfg.solutions:
      ...
      # Note: this is not entirely correct. build.input.gitiles_commit defintion
      # says "The Gitiles commit to run against.". However, here we ignore
      # it if the config specified a revision.
      ...
      revisions[sol.name] = (
          revisions.get(sol.name) or input_commit.id or input_commit.ref)
