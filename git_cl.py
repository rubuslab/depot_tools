...
_CommentSummary = collections.namedtuple(
    '_CommentSummary', ['date_utc', 'message', 'sender',
...
class _RietveldChangelistImpl(_ChangelistCodereviewBase):
  ...
  def GetCommentsSummary(self):
    messages = self.GetIssueProperties().get('messages', [])
    summary = []
...
def CMDcomments(parser, args):
  ...
  summary = sorted(cl.GetCommentsSummary(), key=lambda c: c.date_utc)
  for comment in summary:
    ...
    print('\n%s%s   %s%s\n%s' % (
      color,
      comment.date_utc.strftime('%Y-%m-%d %H:%M:%S UTC'),
      comment.sender,
      Fore.RESET,
  ...
  if options.json_file:
    def pre_serialize(c):
      dct = c.__dict__.copy()
      dct['date'] = dct.pop('date_utc').strftime('%Y-%m-%d %H:%M:%S.%f')
      return dct
