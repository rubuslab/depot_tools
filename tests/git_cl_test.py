...
class CMDUploadTestCase(CMDTestCaseBase):
  def setUp(self):
    ...
    mock.patch('git_cl.fetch_tryjobs').start()
  ...
  def testUploadRetryFailed(self):
    ...
    # patchset.
    git_cl.fetch_tryjobs.side_effect = [
    ...
    ], git_cl.fetch_tryjobs.mock_calls)
    expected_buckets = [
        ('chromium', 'try', 'bot_failure'),
        ('chromium', 'try', 'bot_infra_failure'),
    ]
