...
class SSOAuthenticator(Authenticator):
    """SSOAuthenticator implements a Google-internal authorization
    scheme which uses a git remote helper `git-remote-sso` available to
    Googlers.

    This helper is similar to the publicly available
    `git-remote-persistent-https`.  Googler machines are configured so
    that all *.googlesource.com domains are routed instead to
    *.git.corp.google.com via a secure tunnel which is integrated with
    SSO authentication.

    This class maintains a copy of the git-remote-sso process, parses its
