...
class GcsDependency(Dependency):
    ...
    def GetAndValidateLLVMStampFile(self, output_dir):
        """Returns stamp file path, expected cr_build_revision, and
        whether we can skip downloading."""
        ...
        if os.path.exists(stamp_file_name):
            ...
            skip = False
            print('not skipping')
        return stamp_file_name, expected_cr_build_revision, skip

    def GetOutputDir(self, path_is_download_google_storage_sha):
        """Get the output dir where the tarfile will be extracted

        Supports migrated hooks that used download_google_storage,
        tools/scripts/update.py, and tools/rust/update_rust.py.
        """
        ...
        if path_is_download_google_storage_sha:
            return os.path.dirname(os.path.join(chromium_dir, self.name))
        else:
            # Ex: if name is `src/third_party/llvm-build/Release+Asserts`,
            # output_dir will be `src/third_party/llvm-build/Release+Asserts`.
            return os.path.join(chromium_dir, self.name)
    ...
    def DownloadGoogleStorage(self):
        ...
        path_is_download_google_storage_sha = False
        if self.CheckForHexPath():
            path_is_download_google_storage_sha = True
        output_dir = self.GetOutputDir(path_is_download_google_storage_sha)
        ...
        # This matches the behavior from src/tools/scripts/update.py
        stamp_file_name = ''
        if not path_is_download_google_storage_sha and 'llvm' in self.path:
            stamp_file_name, expected_cr_build_revision, skip = (
                self.GetAndValidateLLVMStampFile(output_dir))
        ...
        dir_to_delete = output_dir
        if path_is_download_google_storage_sha:
          # We want to delete only the extracted dir
          chromium_dir = os.path.dirname(os.getcwd())
          dir_to_delete = os.path.join(chromium_dir, self.name)
        if os.path.exists(dir_to_delete):
          try:
              shutil.rmtree(dir_to_delete)
              print('Removing extract_dir: ', dir_to_delete)
          except OSError:
              print('Nevermind, cant delete ', dir_to_delete)
