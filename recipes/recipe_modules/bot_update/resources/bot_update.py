...
def _git_checkout(sln, sln_dir, revisions, shallow, refs, git_cache_dir,
  ...
  if not pin:
    ...
    while True:
      ...
      # maintainers of *.googlesource.com.
      git(*populate_cmd)
      if _has_in_git_cache(pin, git_cache_dir, url):
        break
      left = time.time() - soft_deadline
      ...
      if attempt >= 2 and left < 0:
        print 'Ran %s seconds past deadline. Aborting.' % (-left)
        # TODO(tandrii): raise exception immediately here, instead of doing
        # useless step 2 trying to fetch something that we know doens't exist
