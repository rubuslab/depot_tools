...
def git_checkouts(solutions, revisions, refs, fetch_tags, git_cache_dir,
                  cleanup_dir):
  build_dir = os.getcwd()
  first_solution = True
  for sln in solutions:
    sln_dir = path.join(build_dir, sln['name'])
    _git_checkout(sln, sln_dir, revisions, refs, fetch_tags, git_cache_dir,
                  cleanup_dir)
    if first_solution:
...
def _git_checkout(sln, sln_dir, revisions, refs, fetch_tags, git_cache_dir,
                  cleanup_dir):
  name = sln['name']
  url = sln['url']
  ...
  if fetch_tags:
    populate_cmd.extend(['--fetch-tags'])
  for ref in refs:
...
def ensure_checkout(solutions, revisions, first_sln, target_os, target_os_only,
                    ...
                    fetch_tags, refs, git_cache_dir, cleanup_dir, gerrit_reset,
  ...
  git_checkouts(solutions, revisions, refs, fetch_tags, git_cache_dir,
                cleanup_dir)
...
def parse_args():
  ...
  parse.add_option(
      '--fetch_checkout_tags',
      action='store_true',
      help=('Fetch tags from the server for the git checkout. '
            'This can slow down fetch considerably when '
            'there are many tags.'))
  ...
  try:
    ...
    try:
      checkout_parameters = dict(
          ...
          # Control how the checkout will occur.
          fetch_tags=options.fetch_checkout_tags,
