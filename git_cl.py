  ...
  def _CMDUploadChange(self, options, git_diff_args, custom_cl_base,
    ...
    if options.squash:
      ...
      if self.GetIssue():  # the issue exists
        ...
        if curr_ps is not None and curr_ps != latest_ps:
          ...
          if ask_for_explicit_yes('Try rebasing on external changes?'):
            # git cl patch tries patching hte whole PS, not diffs between curr/latest PS. wont work here
            # print('Continue syncing with `git cl patch`.')
            # self.CMDPatchWithParsedIssue(
            #    ParseIssueNumberArgument(self.GetIssue()), False, False, False)
            # pass  # instead actually download/apply
            gitiles_client = os.path.join(DEPOT_TOOLS, 'gitiles_client.py')
            # https://chromium.googlesource.com/a/chromium/tools/depot_tools/+/refs/changes/55/3975855/11..refs/changes/55/3975855/13
            with gclient_utils.temporary_file() as diff_tempfile:
              issue = self.GetIssue()
              cmd = [gitiles_client, '-j', diff_tempfile, '-f', 'text', '-u',
                  'https://%s/a/%s/+/refs/changes/%d/%d/%d..refs/changes/%d/%d/%d' % (
                    self._GetGitHost(), self.GetGerritProject(),
                    issue % 100, issue, curr_ps,
                    issue % 100, issue, latest_ps
                  )]
              RunCommand(cmd)
              json_res = json.loads(gclient_utils.FileRead(diff_tempfile))
              value = base64.b64decode(json_res['value'])
              if sys.version_info >= (3,):
                value = value.decode('utf-8')

              with gclient_utils.temporary_file() as patch_tempfile:
                gclient_utils.FileWrite(patch_tempfile, value)
                branch_name = RunGit(['rev-parse', '--abbrev-ref', 'HEAD']).strip()
                # latest_commit = RunGit(['rev-parse', 'HEAD']).strip()
                latest_commit_parent = RunGit(['rev-parse', 'HEAD^']).strip()
                RunGit(['checkout', latest_commit_parent])
                RunGit(['apply', patch_tempfile])
                RunGit(['commit', '-am', 'Incorporate changes from external patchsets %d to %d' % (curr_ps, latest_ps)])
                inserted_commit = RunGit(['rev-parse', 'HEAD']).strip()
                RunGit(['checkout', branch_name])
                RunGit(['rebase', '--onto', inserted_commit, latest_commit_parent, branch_name])

                # if already committed, apply change needs to be committed again.
                #
                # do that for them? 
                # or create commit with git am and rebase?
                #67afa307cd4c88d57f13420548b2d7e372d1607b
                # git rebase --onto third first second
                # git rebase --onto 41dea620ce ed49e1542f77a94649fd374023ad92875c083575 67afa307cd4c88d57f13420548b2d7e372d1607b 
                #41dea620cecbe86e38c8cf91d126232996270b3a
                #     1. create commit "incorporate external changes <ps> to <ps>", parent = curr commit's parent
                #     2. rebase current commit on top of new commit
                # or create a "git cl incorporate" to get ext changes before any commits?
                # if applied, update curr_ps
                # 
                # instead of having a single commit = all ext changes,
                # have commit per new PS, rebase on top
                curr_ps = latest_ps
          else:
      ...
      with gclient_utils.temporary_file() as desc_tempfile:
        ...
        # This section (and above)
        # 1. finds the parent of the original CL, e.g. where the CL in gerrit
        #     is "based" onto
        # 2. finds id of HEAD commit object
        # 3. creates a new commit object, parent = HEAD, to be used to push to gerrit
        ref_to_push = RunGit(
