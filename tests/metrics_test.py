...
class MetricsCollectorTest(unittest.TestCase):
  ...
  def test_version_change_from_opted_out(self):
    ...
    self.print_version_change.assert_not_called()
    self.print_notice.assert_not_called()
    # We don't upload any metrics.
    self.Popen.assert_not_called()
    # We don't modify the config.
    self.FileWrite.assert_not_called()
  ...
  def test_opting_in_by_default_updates_version(self):
    ...
    # We display a notice telling the user metrics collection will take place
    # starting with the next execution, but we no longer display the notice
    # with the changes.
