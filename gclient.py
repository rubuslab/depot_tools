...
class Dependency(gclient_utils.WorkItem, DependencySettings):
  ...
  def ParseGitSubmodules(self):
    """
    Parses git submodules and returns a dict of submodules keyed by path.

    The url is formatted to resemble the format within the DEPS file, i.e.,
    url@commit_hash.
    """
    ...
    with open(filepath) as f:
      for line in f:
        ...
        if m:
          # If line is a submodule section (eg: [submodule "foo"]), create a
          # new submodule.
  ...
  for revision in options.setdep_revisions:
    ...
    if ':' in name:
      ...
      if 'git_dependencies' in local_scope and local_scope[
          'git_dependencies'] in ('SUBMODULES', 'SYNC'):
