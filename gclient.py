...
PREVIOUS_CUSTOM_VARS_FILE = 'GCLIENT_PREVIOUS_CUSTOM_VARS_FILE'
PREVIOUS_CUSTOM_VARS_DEFAULT_FILE = '.gclient_previous_custom_vars'
PREVIOUS_SYNC_COMMITS_FILE = 'GCLIENT_PREVIOUS_SYNC_COMMITS_FILE'
PREVIOUS_SYNC_COMMITS_DEFAULT_FILE = '.gclient_previous_sync_commits'
...
''' % {
  ...
  def _ExtractFileJsonContents(self, expected_file, default_filename):
    # type: (str, str) -> Mapping[str,Any]
    f = expected_file
    if f is None:
    ...
    with open(f, 'r') as open_f:
      print('Reading content from file %s' % f)
      content = open_f.read().rstrip()
      if content:
        return json.loads(content)
    return {}

  def _WriteFileContents(self, expected_file, default_filename, content):
  ...
  def _EnforceSkipSyncRevisions(self, patch_refs):
    ...
    previous_sync_commits = self._ExtractFileJsonContents(
        os.environ.get(PREVIOUS_SYNC_COMMITS_FILE),
        PREVIOUS_SYNC_COMMITS_DEFAULT_FILE)
    ...
    previous_custom_vars = self._ExtractFileJsonContents(
        os.environ.get(PREVIOUS_CUSTOM_VARS_FILE),
        PREVIOUS_CUSTOM_VARS_DEFAULT_FILE)
  ...
  def RunOnDeps(self, command, args, ignore_requirements=False, progress=True):
    ...
    if custom_vars:
      self._WriteFileContents(os.environ.get(PREVIOUS_CUSTOM_VARS_FILE),
                              PREVIOUS_CUSTOM_VARS_DEFAULT_FILE,
                              json.dumps(sorted(custom_vars)))
    ...
    print(os.environ[PREVIOUS_SYNC_COMMITS])
    ...
    self._WriteFileContents(os.environ.get(PREVIOUS_SYNC_COMMITS_FILE),
                            PREVIOUS_SYNC_COMMITS_DEFAULT_FILE,
                            os.environ.get(PREVIOUS_SYNC_COMMITS, '{}'))
