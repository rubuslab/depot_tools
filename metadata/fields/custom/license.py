...
def split_license_value(value: str) -> List[str]:
  ...
  # In some cases, the value delimiter may be in the license type,
  # so check the entire value before splitting by the delimiter.
  if is_license_allowlisted(value):
    return [value]

  licenses = re.split(f" and | or | / |{util.VALUE_DELIMITER}", value)
