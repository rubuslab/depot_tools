...
deps = {
    ...
    'src/' + Var('hello'): Var('git_repo') + '/hello' + '@' + 'hola',
}
...
class SetRevisionTest(unittest.TestCase):
  def setUp(self):
    self.global_scope = {'Var': lambda x: '{%s}' % x}
  ...
  def testSetRevisionWithVarInDepsName(self):
    gclient_eval.SetRevision(
        self.local_scope, 'src/world', 'mundo')
    result = gclient_eval.RenderDEPSFile(self.local_scope)

    self.assertEqual(result, _SAMPLE_DEPS_FILE.replace('hola', 'mundo'))
