...
class _GerritChangelistImpl(_ChangelistCodereviewBase):
  ...
  def CMDUploadChange(self, options, git_diff_args, custom_cl_base, change):
    ...
    try:
      ...
      print(' '.join(['git', 'push', self.GetRemoteUrl(), refspec]))
    ...
    SLOW_PUSH_THRESHOLD = -1
    if execution_time < SLOW_PUSH_THRESHOLD:
      os.remove(git_trace_path)
    else:
      print('Was this upload too slow?\n'
            'When filing a bug please be sure to upload the traces '
            'found at\n'
            '  %s\n' % git_trace_path)
