...
class Dependency(gclient_utils.WorkItem, DependencySettings):
  ...
  def run(self, revision_overrides, command, args, work_queue, options,
    ...
    if run_scm and self.url:
      ...
      patch_branch = patch_branches.pop(
          self.FuzzyMatchUrl(patch_branches), None)
      if command == 'update' and patch_ref is not None:
        self._used_scm.apply_patch_ref(patch_repo, patch_ref, patch_branch,
                                       options, file_list)
...
'''  % {'checkout_path': os.path.join(self.root_dir, dep.name),
  ...
  def _EnforcePatchBranches(self):
    """Checks for patch refs."""
    patch_branches = {}
    if not self._options.patch_branches:
      return patch_branches
    for given_patch_branch in self._options.patch_branches:
      patch_repo, _, patch_branch = given_patch_branch.partition('@')
      if not patch_repo or not patch_branch:
        raise gclient_utils.Error(
            'Wrong revision format: %s should be of the form '
            'patch_repo@patch_ref.' % given_patch_branch)
      patch_branches[patch_repo] = patch_branch
    return patch_branches
  ...
  def RunOnDeps(self, command, args, ignore_requirements=False, progress=True):
    ...
    if command == 'update':
      ...
      patch_branches = self._EnforcePatchBranches()
    ...
    work_queue.flush(revision_overrides, command, args, options=self._options,
                     patch_refs=patch_refs, patch_branches=patch_branches)
    ...
    if patch_branches:
      raise gclient_utils.Error(
          'The following --patch-branch flags were not used. '
          'Please fix it:\n%s' %
          ('\n'.join(
              patch_repo + '@' + patch_branch
              for patch_repo, patch_branch in patch_branches.iteritems())))
...
def CMDsync(parser, args):
  ...
  parser.add_option('--patch-branch', action='append',
                    dest='patch_branches', metavar='PATCH_BRANCH', default=[],
                    help='The branch to apply the patch on top of, in the '
                         'format dep@branch. If not given, and a branch was '
                         'specifed as part of a --revision flag, we will use '
                         'that. Otherwise, gclient will iterate over all '
                         'remote branches and select one that contains the '
                         'revision the solution is synced at.')
