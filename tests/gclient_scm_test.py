...
class CheckDiffTest(fake_repos.FakeReposTestBase):
  ...
  def testCheckDiff(self):
    """Correctly check for diffs in DEPS."""
    scm = gclient_scm.GitWrapper(self.url, self.root_dir, '.')
    file_list = []
    ...
    self.options.revision = None
    scm.update(self.options, None, file_list)
    self.assertEqual(self.githash('repo_1', 3), self.gitrevparse(self.root_dir))

    self.assertFalse(scm.deps_diff(self.githash('repo_1', 1)))
    self.assertTrue(scm.deps_diff(self.githash('repo_1', 2)))
