...
LUCI_AUTH_UNSUPPORTED_PLATFORM = ['aix']
...
def _is_supported_platform():
    ...
    return not any(map(sys.platform.startswith, LUCI_AUTH_UNSUPPORTED_PLATFORM))
...
def run_gsutil(target, args, clean=False):
    ...
    if _is_luci_context() or not _is_supported_platform():
