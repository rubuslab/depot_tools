...
def main():
    ...
    parser.add_argument(
        ...
        "--reviewer",
        action="append",
        help="To specify multiple reviewers, either use a comma separated list, e.g. "
    ...
    try:
        if not args.ignore_dirty_tree and not is_pristine(current_dir):
            raise Error(
                "Ensure %s is clean first (no non-merged commits)."
                % current_dir
            )
        ...
        for dependency in dependencies:
            ...
            if not os.path.isdir(full_dir):
                ...
                if not os.path.isdir(full_dir):
                    raise Error(
                        "Directory not found: %s (%s)" % (dependency, full_dir)
                    )
    ...
    if not reviewers:
        print(
            "You forgot to pass -r, make sure to insert a R=foo@example.com line"
        )
