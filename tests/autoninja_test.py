...
class AutoninjaTest(trial_dir.TestCase):
    ...
    def test_is_corp_machine_using_external_account(self):
        with mock.patch('autoninja._is_corp', return_value=False):
            self.assertFalse(
                autoninja._is_corp_machine_using_external_account())

        with mock.patch('autoninja._is_corp', return_value=True), mock.patch(
                'autoninja._account', return_value='foo@google.com'):
            self.assertFalse(
                autoninja._is_corp_machine_using_external_account())

        with mock.patch('autoninja._is_corp', return_value=True), mock.patch(
                'autoninja._account', return_value='foo@chromium.org'):
            self.assertTrue(autoninja._is_corp_machine_using_external_account())
