...
class TryserverApi(recipe_api.RecipeApi):
  ...
  def set_change(self, change):
    ...
    assert change.host.endswith(gs_suffix) is True
    self._gerrit_info_initialized = False
    self._gerrit_change = change
    gerrit_host = '%s.googlesource.com' % change.host[:-len(gs_suffix)]
    self._gerrit_change_repo_url = 'https://%s/%s' % (gerrit_host,
                                                      change.project)
