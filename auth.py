...
# TODO(crbug.com/1001756): Remove. JSON file with token must define client_id
# and client_secret, since the refresh token is associated with them (so using
# some other client ID/secret when refreshing will not work).
...
class Authenticator(object):
  ...
  def _load_access_token(self):
    ...
    If using an external refresh token, creates new credentials from it, and
    returns a freshly refreshed AccessToken.
    Otherwise returns an AccessToken from luci-auth.
    """
