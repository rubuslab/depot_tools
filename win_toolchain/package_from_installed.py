  ...
  for root, _, files in os.walk(sdk_path):
    for f in files:
      ...
      if VS_VERSION == '2017' and combined.endswith('dbghelp.dll'):
        prefix_path = os.path.join(vs_path, 'Common7', 'IDE', 'CommonExtensions',
                                  'Microsoft', 'TestWindow', 'Extensions')
        arch_dir = 'x64' if combined.count('\\x64\\') > 0 else ''
        good_dbghelp_path_candidates = [
          os.path.join(prefix_path, 'CppUnitFramework', arch_dir, 'dbghelp.dll'),
