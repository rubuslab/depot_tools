...
class BotUpdateTestApi(recipe_test_api.RecipeTestApi):
    ...
    def output_json(
        ...
        --output-json option.
        """
        output = {"did_run": True, "patch_failure": False}
        ...
        if fixed_revisions:
            for project_name, revision in fixed_revisions.items():
                ...
                elif revision.startswith("refs/") or revision.startswith(
                    "origin/"
                ):
                    revision = self.gen_revision(
