...
try:
  ...
  with open(os.path.join(output_dir, 'args.gn')) as file_handle:
    for line in file_handle:
      ...
      match_use_goma = re.match('^\s*use_goma\s*=\s*true(\s*$|\s*#.*$)', line)
      if match_use_goma and 'GOMA_DISABLED' not in os.environ:
        use_goma = True
        continue
      match_use_jumbo_build = re.match(
          '^\s*use_jumbo_build\s*=\s*true(\s*$|\s*#.*$)', line)
...
if not j_specified and not t_specified:
  if use_goma:
    ...
    if core_addition:
      ...
      GB = 1024 * 1024 * 1024
      j_value = min(num_cores, physical_ram / (2 * GB))
    args.append('-j')
    args.append('%d' % j_value)
