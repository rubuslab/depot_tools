  ...
  def _CMDUploadChange(self, options, git_diff_args, custom_cl_base,
    ...
    if options.squash:
      ...
      if self.GetIssue():  # the issue exists
        ...
        if curr_ps != latest_ps:
          # prompt user:
          # 1. sync, try incorporating/cherry picking latest external changes
          #     1. if succeed, upload
          #     2. if fail or have merge conflicts, exit and tell user to try again
          # 2. continue anyway?
          # 2. or exit?
          #
          # (also hint about using new command to proactively sync latest external changes)
          print('External changes have been published to %s.' %
                self.GetIssueURL())
          if ask_for_explicit_yes('Try patching external changes?'):
    ...
    if options.squash:
      ...
      try:
        # Gerrit will not update fast enough to return the correct patchset
        # number, i.e. the patchset we're uploading now. Manually increment.
        self.SetPatchset(self.GetMostRecentPatchset() + 1)
      except:
        print('unable to set patchset, setting to 1')
        self.SetPatchset(1)
