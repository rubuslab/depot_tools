...
def force_solution_revision(solution_name, git_url, revisions, cwd):
  ...
  if revision and revision.upper() != 'HEAD':
    ...
    what = branch if branch.startswith('refs/') else 'origin/%s' % branch

  # Note that -- argument is necessary to ensure that git treats
  # `what` argument as revision or ref, and not as a file/directory which
  # happens to have the exact same name.
  git('checkout', '--force', what, '--', cwd=cwd)
