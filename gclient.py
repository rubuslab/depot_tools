    ...
    if run_scm and self.url:
      ...
      if command == 'update':
        if patch_ref is not None:
          ...
          latest_commit = self._used_scm.revinfo()
        os.environ[PREVIOUS_SYNC_COMMITS] += '%s:%s,' % (self.name,
                                                         latest_commit)
...
''' % {
  ...
  def _EnforceSkipSyncRevisions(self, patch_refs):
    ...
    previous_sync_commits = os.environ.get(PREVIOUS_SYNC_COMMITS, '')
  ...
  def RunOnDeps(self, command, args, ignore_requirements=False, progress=True):
    ...
    if command == 'update':
      ...
      # Clear PREVIOUS_SYNC_COMMITS for the queue to re-fill with updated
      # commits.
      os.environ[PREVIOUS_SYNC_COMMITS] = ''
