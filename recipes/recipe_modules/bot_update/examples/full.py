...
def RunSteps(api):
    ...
    if api.properties.get("deprecated_got_revision_mapping"):
        ...
        api.gclient.c.got_revision_reverse_mapping[
            "got_angle_revision"
    ...
    step_test_data = None
    bot_update_output = engine_types.thaw(
    ...
    if api.properties.get("resolve_chromium_fixed_version"):
        api.bot_update.resolve_fixed_revision(
...
def GenTests(api):
    ...
    yield (
        api.test("tryjob_fail_patch", status="FAILURE")
        + try_build()
        + api.properties(fail_patch="apply")
        + api.step_data("bot_update", retcode=88)
    )
    yield (
        api.test("tryjob_fail_patch_download", status="INFRA_FAILURE")
        + try_build()
    ...
    yield (
        api.test("upload_traces", status="FAILURE")
        + try_build()
        + api.properties(fail_patch="apply")
        + api.step_data("bot_update", retcode=88)
    )
    ...
    yield (
        api.test("upload_traces_fail", status="FAILURE")
        + try_build()
        + api.properties(fail_patch="apply")
        + api.step_data("bot_update", retcode=88)
        + api.step_data(
