...
def CMDdiff(parser, args):
  ...
  base = self._GitGetBranchConfigValue('last-upload-hash')
  if not base:
    base = self._GitGetBranchConfigValue('gerritsquashhash')
  if not base:
    detail = self._GetChangeDetail(['CURRENT_REVISION', 'CURRENT_COMMIT'])
    revision_info = detail['revisions'][detail['current_revision']]
    fetch_info = revision_info['fetch']['http']
    RunGit(['fetch', fetch_info['url'], fetch_info['ref']])
    base = 'FETCH_HEAD'
