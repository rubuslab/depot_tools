...
def main():
    ...
    parser.add_argument('--extra-step-types',
    ...
    if args.extra_step_types:
      # Make room for the extra build types.
      global long_ext_count
      long_ext_count += len(args.extra_step_types.split(';'))

    try:
      with open(log_file, 'r') as log:
        entries = ReadTargets(log, False)
        SummarizeEntries(entries, args.extra_step_types)
    except IOError:
