...
def _find_next_bazel_in_path() -> Path:
  ...
  for path in path_env.split(os.pathsep):
    path = Path(path)
    if path.resolve() != depot_tools:
      search_paths.append(path)
  new_path_env = os.pathsep.join(str(x) for x in search_paths)
...
# All functions used to search for Bazel.
_SEARCH_FUNCTIONS = [
    _find_bazel_cros,
