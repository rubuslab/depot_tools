...
class GitWrapper(SCMWrapper):
  ...
  def _Clone(self, revision, url, options):
    ...
    if in_cog_workspace:
      ...
      # If the URL looks like chromium source, fetch a little extra history.
      # This works around issues with tools like lastchange.py that look for
      # tags in commit messages that may not be present at HEAD.
      depth = 5 if url.endswith('src.git') else 1
      revision = self._AutoFetchRef(options, revision, depth=depth)
      remote_ref = scm.GIT.RefToRemoteRef(revision, self.remote)
