...
::
:: When modifying this script, run the following test scenario. After each
:: installation, re-run this script to ensure that it is a no-op.
:: - Cold (git clean -x -f -d .) installation.
::   - Run "python.bat" in another shell, keep it open, add ".bleeding_edge"
::     and test warm downgrade.
::   - Close the Python shell, run again, ensure that old directory is cleaned.
:: - Cold w/ ".bleeding_edge" installation.
::   - Run "python.bat" in another shell, keep it open, delete ".bleeding_edge"
::     and test warm downgrade.
::   - Close the Python shell, run again, ensure that old directory is cleaned.
:: - Run "git bash", ensure that the shell works.
::   - Run "python" in the shell, ensure that an interpreter opens.
::   - Run "git map-branches" in the shell, ensure that this works.
...
set WIN_TOOLS_EXTRA_ARGS=--verbose
...
:: stripping all text preceding "versison:". This leaves us with the version
:: string.
::
