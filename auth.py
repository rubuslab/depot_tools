...
class _BadLuciContextParameters(Exception):
  """LUCI context JSON parameters are missing or not valid."""


def _read(section_key):
...
def _load_local_auth():
  ...
  Raises:
    _BadLuciContextParameters: file is missing or not valid.
  """
  data = _read('local_auth')
  if data is None:
    raise _BadLuciContextParameters('Missing "local_auth" in LUCI_CONTEXT')
  try:
    ...
    raise _BadLuciContextParameters(
...
def get_luci_context_access_token():
  ...
  """
  try:
    local_auth = _load_local_auth()
  except _BadLuciContextParameters as e:
    logging.error('Error in loading local auth: %s', str(e))
    return None
