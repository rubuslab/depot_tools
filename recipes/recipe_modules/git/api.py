...
class GitApi(recipe_api.RecipeApi):
  ...
  def url_for_path(self, path):
    """Returns (str): A URL pointing to a repository at "path".

    Args:
      path (Path): The Path of the repository.
    """
    return 'file://%s' % (str(path),)
  ...
  def rev_parse(self, path, ref=None, name=None, test_data=None):
    """Resolves the specified ref via "rev-parse".

    Args:
      path (Path): the path to the repository.
      ref (str or None): The ref to resolve, or None to resolve HEAD.
      name (str or None): Step name to use, or None to generate one.
      test_data (str or None): If supplied, use this value as output test data
          for the step.

    Returns (str): The resolved commit.
    """
    ref = ref or 'HEAD'
    test_data = test_data or 'deadbeef'
    with self.m.context(cwd=path):
      result = self(
          'rev-parse', ref,
          stdout=self.m.raw_io.output(),
          name=name or 'git rev-parse %s' % (ref,),
          step_test_data=lambda: self.m.raw_io.test_api.stream_output(
              test_data or 'deadbeef'),
      )
  ...
  def checkout(self, url, ref=None, dir_path=None, recursive=False,
               ...
               submodule_update_force=False, submodule_update_recursive=False):
    """Performs a full git checkout and returns sha1 of checked out revision.
    ...
    Args:
      ...
      submodule_update_force: Ignored, TODO remove. Force is enabled for all
          submodule operations by default.
      submodule_update_recursive: Synonym for "recursive=True" TODO remove. All
          Setting "recursive" to True means that all submodules will recursively
          update by defualt.

    Returns: If the checkout was successful, this returns the commit hash of
      the checked-out-repo. Otherwise this returns None.
    """
    recursive = recursive or submodule_update_recursive
    ...
    checkout_commit = self.ensure_checkout(
        repo,
        ref=ref,
        recursive=recursive,
    ...
    self.m.step.active_result.presentation.step_text = (
        "<br/>checked out %r<br/>" % checkout_commit)
    if set_got_revision:
      self.m.step.active_result.presentation.properties['got_revision'] = (
          checkout_commit)
