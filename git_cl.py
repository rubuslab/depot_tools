...
def CMDformat(parser, args):
  ...
  if not opts.presubmit:
    for diff_xml in GetDiffXMLs(diff_files):
      xml_dir = GetDirtyMetricsDirs(diff_xml)
      if xml_dir:
        ...
        # If in the histograms directory, get the list of paths to individual
        # XML files that have been changed, run pretty_print command for each
        # file as pretty_print.py in histograms needs a relative path argument.
...
def GetDirtyMetricsDirs(diff_xml):
  metrics_xml_dirs = [
