...
class CookiesAuthenticator(Authenticator):
  ...
  def _get_netrc(cls):
    ...
    with tempdir() as tdir:
      ...
      empty_path = os.path.join(tdir, 'empty_netrc')
      with open(empty_path, 'w') as fd:
        pass
      return cls._get_netrc_from_path(netrc_path, empty_path)

  @classmethod
  def _get_netrc_from_path(cls, path, empty_path):
    try:
      return netrc.netrc(path)
    except IOError:
      print >> sys.stderr, 'WARNING: Could not read netrc file %s' % path
      return netrc.netrc(empty_path)
