...
class TraceConfig:
    ...
    def gen_id(self, regen=False) -> None:
        ...
        if regen or self._uuid_stale():
            self._config.set(TRACE_SECTION_KEY, KEY_USER_UUID,
                             str(uuid.uuid4()))
            self._config.set(
                TRACE_SECTION_KEY,
    ...
    def set_batch(self, enabled: bool) -> None:
        ...
        self._config.set(TRACE_SECTION_KEY, BATCH_PUBLISHING_ENABLED_KEY,
                         str(enabled))
...
class RootConfig:
    ...
    def update(self, notice_countdown: int) -> None:
        """Update the config."""
        self._config.set(ROOT_SECTION_KEY, NOTICE_COUNTDOWN_KEY,
                         str(notice_countdown))
