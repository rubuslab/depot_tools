...
def _RunMojomFormat(opts, paths, top_dir, upstream_commit):
    ...
    if not chromium_src_path:
        DieWithError('Could not find the Chromium checkout')
    mojom_format_path = os.path.join(chromium_src_path, 'mojo', 'public',
                                     'tools', 'mojom', 'mojom_format.py')
    if not os.path.exists(mojom_format_path):
        DieWithError(f'Could not find {mojom_format_path}')
