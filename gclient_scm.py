    ...
    def _DeleteOrMove(self, force):
        ...
        if force and os.environ.get("CHROME_HEADLESS") == "1":
            ...
            gclient_utils.AddWarning(
                "Conflicting directory %s deleted." % self.checkout_path
            )
            gclient_utils.rmtree(self.checkout_path)
        else:
            bad_scm_dir = os.path.join(
                self._root_dir, "_bad_scm", os.path.dirname(self.relpath)
            )
            ...
            dest_path = tempfile.mkdtemp(
                prefix=os.path.basename(self.relpath), dir=bad_scm_dir
            )
            self.Print(
...
class GitWrapper(SCMWrapper):
    ...
    """
        ...
        for pr in patch_revs_to_process:
            ...
            if not options.rebase_patch_ref:
                ...
                self.Print(
                    "Will cherrypick %r .. %r on top of %r."
    ...
    def set_config(f):
        def wrapper(*args):
            ...
            if os.path.exists(os.path.join(args[0].checkout_path, ".git")):
                ...
                config = (
                    subprocess2.capture(
                        ["git", "config", "-l"], cwd=args[0].checkout_path
                    )
                    .decode("utf-8")
                    .strip()
                    .splitlines()
    ...
    def update(self, options, args, file_list):
        ...
        if not revision:
            ...
            revision = scm.GIT.GetRemoteHeadRef(
                self.checkout_path, self.url, self.remote
            )
        ...
        if verbose:
            self.Print(
                "Checked out revision %s" % self.revinfo(options, (), None),
                timestamp=False,
            )
    ...
    def _CheckMinVersion(min_version):
        ...
        if not ok:
            raise gclient_utils.Error(
                "git version %s < minimum required %s"
    ...
    def _SetFetchConfig(self, options):
        ...
        if options.force or options.reset:
            try:
                self._Run(
                    ["config", "--unset-all", "remote.%s.fetch" % self.remote],
                    options,
                )
                self._Run(
                    [
