  ...
  def ensure_checkout(self,
    ...
    Args:
      ...
      * accept_input_gerrit_changes: if True, get the patchset from
        buildbucket.build.input.gerrit_changes, with an assertion that the size
        of buildbucket.build.input.gerrit_changes is one, because bot_update
        module ONLY supports one CL. Users may specify a CL via
        tryserver.set_change() and explicitly set this flag False.
    """
