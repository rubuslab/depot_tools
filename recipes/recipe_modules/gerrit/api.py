...
class GerritApi(recipe_api.RecipeApi):
    ...
    def get_change_description(
        self, host, change, patchset, timeout=None, step_test_data=None
    ):
        ...
        ri = self.get_revision_info(
            host, change, patchset, timeout, step_test_data
        )
    ...
    def get_related_changes(
        self, host, change, revision="current", step_test_data=None
    ):
        ...
        if not step_test_data:
            step_test_data = (
                lambda: self.test_api.get_related_changes_response_data()
            )
    ...
    def abandon_change(
        self, host, change, message=None, name=None, step_test_data=None
    ):
        args = [
            "abandon",
            "--host",
    ...
    def move_changes(
        self, host, project, from_branch, to_branch, step_test_data=None
    ):
        args = [
            "movechanges",
            "--host",
    ...
    def update_files(
        ...
        """
        assert (
            len(new_contents_by_file_path) > 0
        ...
        if submit or submit_later:
            self(
                "set Bot-Commit+1 for change %d" % change,
                [
                    "setbotcommit",
                    "--host",
                    host,
