...
def UploadAll(options, args, orig_args):
  ...
  while upstream_branch_name not in ('master', 'main'):
    ...
    _, upstream_branch = cl.FetchUpstreamTuple(cl.GetBranch())
    upstream_branch_name = scm.GIT.ShortBranchName(upstream_branch)
    if upstream_branch_name not in ('master', 'main'):
    ...
    messages = []
    if rebase_needed:
      messages.append('Stacked changes branches have diverged.')
    else:
      messages.append('Branch')

  # if so prompt user if they would like to upload all or just one
  messages = []
  if diverged:
    messages.append('branches have diverged')
  if outdated_upstream:
    message.append('Would you like to upload the entire chain?')

  if messages:
    answer = gclient_utils.AskForData(
        'TODO: Would you like to upload the entire chain? (')


  if upload_current_only:
