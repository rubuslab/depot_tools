  ...
  def _upload_traces(self):
    with self.m.step.nest('upload git traces') as presentation:
      zip_path = (self.m.archive.package(self._trace_dir).archive(
          'compress traces', self.m.path.join(self._trace_dir, 'traces.zip')))
      try:
        self.m.gsutil.upload(name='upload',
                             source=zip_path,
                             bucket='chrome-bot-traces',
                             dest='%s.zip' % self.m.buildbucket.build.id)
