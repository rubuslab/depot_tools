...
class SSOAuthenticator(Authenticator):
    ...
    def _launch_sso_helper(cls) -> SSOInfo:
        ...
        try:
            ...
            # We close stdin after we finish reading and parsing the config so that
            # the helper knows to clean up all its extra output files.
            proc.stdin.close()
            timer.cancel()
    ...
    def authenticate(self, conn: HttpConn):
        ...
        if 'Cookie' not in conn.req_headers:
          print(sso_info.cookies._cookies)
          print(conn.req_uri)
          print(conn.req_host)
          print(conn.req_headers)
