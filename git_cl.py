...
def _ComputeDiffLineRanges(files, upstream_commit):
  ...
  lines = '-U%s' % settings.GetFormatLinesOfContext()
...
class Settings(object):
  ...
  def GetFormatLinesOfContext(self):
    return (self._GetConfig('rietveld.format-lines-of-context', error_ok=True)
            or "0")
...
def LoadCodereviewSettingsFromFile(fileobj):
  ...
  SetProperty('run-post-upload-hook', 'RUN_POST_UPLOAD_HOOK',
              unset_error_ok=True)
  SetProperty(
      'format-lines-of-context', 'FORMAT_LINES_OF_CONTEXT', unset_error_ok=True)
