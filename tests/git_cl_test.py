...
class TestGitCl(unittest.TestCase):
  ...
  def _cmd_set_quick_run_gerrit(self, notify=None):
    self.mockGit.config['branch.master.gerritissue'] = '123'
    self.mockGit.config['branch.master.gerritserver'] = (
        'https://chromium-review.googlesource.com')
    self.mockGit.config['remote.origin.url'] = (
    ...
    self.calls = [
        (('SetReview', 'chromium-review.googlesource.com',
          'infra%2Finfra~123', None,
          {'Commit-Queue': 1, 'Quick-Run': 1}, notify, None), ''),
    ]
  ...
  def test_cmd_set_quick_run_gerrit(self):
    self._cmd_set_quick_run_gerrit(notify=False)
...
class CMDTryTestCase(CMDTestCaseBase):
  ...
  @mock.patch('git_cl.Changelist.SetCQQRState')
  def testSetCQDryRunByDefault(self, mockSetCQQRState):
    mockSetCQQRState.return_value = 0
    self.assertEqual(0, git_cl.main(['try']))
    git_cl.Changelist.SetCQQRState.assert_called_with(git_cl._CQState.DRY_RUN)
    self.assertEqual(
        sys.stdout.getvalue(),
  ...
  @mock.patch('git_cl.Changelist.SetCQQRState')
  def testSetCQQuickRunByDefault(self, mockSetCQQRState):
    mockSetCQQRState.return_value = 0
    self.assertEqual(0, git_cl.main(['try', '-q']))
    git_cl.Changelist.SetCQQRState.assert_called_with(
        git_cl._CQState.DRY_RUN, git_cl._QRState.QUICK_RUN)
    self.assertEqual(
        sys.stdout.getvalue(),
