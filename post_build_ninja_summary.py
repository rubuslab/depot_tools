...
def ReadTargets(log, show_all):
    ...
    for line in log:
        ...
        if not show_all and end < last_end_seen:
            ...
            targets_dict = {}
            #print "Resetting due to end time - %f versus %f, %s" % (end, last_end_seen, cmdhash)
        target = None
        if cmdhash in targets_dict:
          ...
          if not show_all and (target.start != start or target.end != end):
            ...
            # the end times may not got backwards so the last build may not be
            ...
            #print "Resetting due to duration %f versus %f, %f versus %f, %s" % (target.start, start, target.end, end, cmdhash)
            targets_dict = {}
            target = None
        if not target:
