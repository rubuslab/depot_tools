...
"""Creates a new branch for each file modified in the current branch. Uploads a
CL for each created branch."""
...
def CreateBranchForFile(branch_prefix, file_path):
  branch = branch_prefix + '_' + os.path.splitext(file_path.split("/")[-1])[0]
  for i in range(0, 10):
    try:
...
def FormatDescriptionOrComment(txt, root, file_path):
  ...
  txt = txt.replace('$file_name', file_path.split("/")[-1])
  return txt

def UploadClForFile(root, main_branch, file_path, description, comment):
  ...
  owners_finder_instance = owners_finder.OwnersFinder(
      [file_path], root, None, fopen=file, os_path=os.path)
...
def main():
  ...
  try:
    ...
    for file_path in file_paths:
      UploadClForFile(root, main_branch, file_path, description, comment)
