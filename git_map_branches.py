...
class BranchMapper(object):
  ...
  def __is_dormant_branch(self, branch):
    if '/' in branch:
      return False
    # We can't simply use config --get branch.{}.dormant because it returns
    # an error level of 1 when the config isn't present.
    config = run('config', '--get-regexp', 'branch.{}'.format(branch))
    for config_line in config.split(os.linesep):
      (key, value) = config_line.split(' ')
      if key == 'branch.{}.dormant'.format(branch):
        return value == 'true'
    return False
