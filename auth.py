...
def get_luci_context_access_token():
  ...
  def authErr(msg, e=None, *args):
    error_msg = msg % args
    logging.exception(error_msg)
    if not e:
      raise LuciContextAuthError(error_msg)
    raise LuciContextAuthError('%s: %s' % (error_msg, e))

  try:
    with open(ctx_path) as f:
      loaded = json.load(f)
  except (OSError, IOError, ValueError) as ex:
    authErr('Failed to open, read or decode LUCI_CONTEXT', e=ex)
  try:
    local_auth = loaded.get('local_auth')
  except AttributeError as ex:
    authErr('LUCI_CONTEXT not in proper format', e=ex)
  ...
  try:
    ...
    authErr('local_auth: unexpected local auth format', e=ex)

  # if account_id is not specified, LUCI_CONTEXT should not be picked up
  if not account_id:
    return None
  ...
  try:
    ...
    authErr('local_auth: Unexpected access token response format', e=ex)
  if error_code:
  ...
  try:
    ...
    authErr('Invalid expiry in returned token', e=ex)
  logging.debug(
  ...
  if not _validate_luci_context_access_token(access_token):
    authErr('local_auth: the returned access token is invalid')
  return access_token
