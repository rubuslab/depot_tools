...
def _data_exists(input_sha1_sum, output_filename, extract, force):
...
def download_from_google_storage(
  ...
  if all(_data_exists(sha1, path, extract, force) for sha1, path in input_data):
    return 0

  # Call this once to ensure it exists. Note, this causes a network call. Any
  # fast bailout should be done before this point.
