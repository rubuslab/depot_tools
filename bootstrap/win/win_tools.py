...
class Template(collections.namedtuple('Template', (
    'WIN_TOOLS_RELDIR',
    'PYTHON_RELDIR', 'PYTHON_BIN_RELDIR_UNIX',
    'GIT_BIN_DIR', 'GIT_BIN_DIR_UNIX', 'GIT_PROGRAM',
    ))):
  ...
  def maybe_install(self, name, dst_path):
    ...
    return _maybe_update(t.safe_substitute(self._asdict()), dst_path)


def _maybe_update(content, dst_path):
...
def maybe_copy(src_path, dst_path):
  with open(src_path, 'r') as fd:
    content = fd.read()
  return _maybe_update(content, dst_path)
...
def ensure_git(args, template):
  ...
  git_postprocess(args, template, git_directory)
...
def git_postprocess(args, template, git_directory):
...
def main(argv):
  ...
  if not args.win_tools_name:
    ...
    template = template._replace(
        WIN_TOOLS_RELDIR=args.win_tools_name,
        PYTHON_RELDIR=os.path.join(args.win_tools_name, 'python'),
        PYTHON_BIN_RELDIR_UNIX=posixpath.join(args.win_tools_name, 'python', 'bin'),
        GIT_BIN_DIR=os.path.join(args.win_tools_name, 'git'),
        GIT_BIN_DIR_UNIX=posixpath.join(args.win_tools_name, 'git'))
    ...
    template.maybe_install(
        'win_tools_reldir.new.txt',
        os.path.join(ROOT_DIR, 'win_tools_reldir.txt'))
    template.maybe_install(
        'python27.new.bat',
        os.path.join(ROOT_DIR, 'python.bat'))

    win_tools_dir = os.path.join(ROOT_DIR, args.win_tools_name)
    git_postprocess(args, template, os.path.join(win_tools_dir, 'git'))
