...
class ChangelistTest(unittest.TestCase):
  ...
  def testPostUploadUpdates(
      self, mockAddReviewers, mockSetHashTags, mockRunPostHook, *_mocks):
    ...
    new_upload = git_cl._NewUpload(
        reviewers, ccs, 'pushed-commit', 'last-uploaded-commit',
        'parent-commit', change_desc)

    cl.PostUploadUpdates(options, new_upload, '12345')
    logging.warning(self.mockGit.config)

    mockAddReviewers.assert_called_once_with(
        'chromium', 'project~123456',
        reviewers=reviewers, ccs=ccs, notify=False)
    mockSetHashTags.assert_called_once_with(
        'chromium', 'project~123456', add_hashtags={'bok-bok', 'stonks'})
    mockRunPostHook.assert_called_once_with(
        True, 'parent-commit', change_desc.description, True)
