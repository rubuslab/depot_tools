...
class CMDFormatTestCase(unittest.TestCase):
  ...
  def _read_temp_file(self, fname):
    return gclient_utils.FileRead(os.path.join(self._top_dir, fname))
  ...
  def testJSONformatDiff(self):
    ...
    self.assertEqual(
        ...
        # Ignore the windows carriage returns that might be added
        self._read_temp_file(test_file).replace('\r', ''))
