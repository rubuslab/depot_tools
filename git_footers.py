...
def add_footer(message, key, value, after_keys=None, before_keys=None):
  ...
  If before_keys and after_keys are None (default), appends footer last.
  ...
  as *early* as possible while still appearing after all provided keys.
  If both before_keys and after_keys are provided, will try to satisfy the
  constraint. If the constraint is unsatisfiable, after_keys takes precedence.
  ...
  if not footer_lines:
    ...
    before_indices = [
        ...
        if normalize_name(parse_footer(x)[0]) == k]
    if after_indices and before_indices:
